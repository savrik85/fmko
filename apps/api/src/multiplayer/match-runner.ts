    ...
    for (const match of matches.results) {
        ...
        try {
            ...
            try {
                for (const [teamDbId, lineup, idMap] of [
                    ...
                    const relRows = await db.prepare(
                        `SELECT player_a_id, player_b_id, type
                         FROM relationships
                         WHERE player_a_id IN (${placeholders})
                            OR player_b_id IN (${placeholders})`
                    ).bind(...dbPlayerIds, ...dbPlayerIds).all().catch((e) => {
                        logger.warn({module: "match-runner"}, "relationships query", e);
                        return {results: []};
                    });
                    ...
                    for (const p of lineup) {
                        ...
                        for (const r of relRows.results as Array<{
                            player_a_id: string;
                            player_b_id: string;
                            type: string
                        }>) {
            ...
            const stadiumRow = await db.prepare("SELECT * FROM stadiums WHERE team_id = ?")
                .bind(homeTeamId).first<Record<string, unknown>>().catch((e) => {
                    logger.warn({module: "match-runner"}, "Failed to load stadium", e);
                    return null;
                });
            ...
            ).bind(homeTeamId).first<{ population: number; size: string; reputation: number }>().catch((e) => {
                logger.warn({module: "match-runner"}, "Failed to load home village info", e);
                return null;
            });
            ...
            const busRows = await db.prepare(
                ...
                .catch((e) => {
                    logger.warn({module: "match-runner"}, "load bus subsidies for attendance", e);
                    return {results: [] as Array<{ bus_size: keyof typeof BUS_CONFIG.SIZES }>};
                });
            let busDropIn = 0;
            ...
            const recentWins = await db.prepare(
                `SELECT COUNT(*) as w
                 ...
                 WHERE win = 1`
            ).bind(homeTeamId, homeTeamId, homeTeamId, homeTeamId).first<{ w: number }>().catch((e) => {
                logger.warn({module: "match-runner"}, "Failed to load recent wins", e);
                return {w: 0};
            });
            ...
            if (allLineupIds.length > 0) {
                const celebRows = await db.prepare(
                    `SELECT is_celebrity, personality
                     FROM players
                     WHERE id IN (${allLineupIds.map(() => "?").join(",")})
                       AND is_celebrity = 1`
                ).bind(...allLineupIds).all().catch((e) => {
                    logger.warn({module: "match-runner"}, "celeb attendance check", e);
                    return {results: []};
                });
                for (const celebRow of celebRows.results) {
                    ...
                    const typeBonus: Record<string, number> = {
                        legend: bonusMap[pers.celebrityTier] ?? 1.5,
                        fallen_star: 1.3,
                        glass_man: 1.4
                    };
            ...
            const fansSatRow = await db.prepare("SELECT satisfaction FROM fans WHERE team_id = ?")
                .bind(homeTeamId).first<{ satisfaction: number }>().catch((e) => {
                    logger.warn({module: "match-runner"}, "load fans satisfaction", e);
                    return null;
                });
            ...
            const promoRow = await db.prepare("SELECT promotion_boost FROM matches WHERE id = ?")
                .bind(matchId).first<{ promotion_boost: number }>().catch((e) => {
                    logger.warn({module: "match-runner"}, "load promotion", e);
                    return null;
                });
            ...
            const acceptedOfficials = await db.prepare(
                `SELECT COUNT(*) as cnt
                 ...
                 WHERE match_id = ?
                   AND team_id = ?
                   AND status IN ('accepted', 'attended')`
            ...
            ).first<{ id: string }>().catch((e) => {
                logger.warn({module: "match-runner"}, "Failed to load active season", e);
                return null;
            });
                ...
                for (const u of allUpdates) {
                    if (u.yellowCards > 0) {
                        const stats = await db.prepare("SELECT COALESCE(SUM(yellow_cards), 0) as yellow_cards FROM player_stats WHERE player_id = ? AND season_id = ?")
                            .bind(u.playerId, season.id).first<{ yellow_cards: number }>().catch((e) => {
                                logger.warn({module: "match-runner"}, "query failed", e);
                                return null;
                            });
                        if (stats && stats.yellow_cards > 0 && stats.yellow_cards % 4 === 0) {
                            await db.prepare("UPDATE players SET suspended_matches = suspended_matches + 1 WHERE id = ?")
                                .bind(u.playerId).run().catch((e) => {
                                    logger.warn({module: "match-runner"}, "op failed", e);
                                });
                        }
                    }
                }
...
export async function buildMatchPlayers(
    ...
    if (options?.matchKey) {
        try {
            ...
            const dayBeforeRng = createRng(absenceSeedForMatch({
                matchKey: options.matchKey,
                teamId,
                phase: "day_before"
            }));
            const matchDayRng = createRng(absenceSeedForMatch({
                matchKey: options.matchKey,
                teamId,
                phase: "match_day"
            }));
            ...
            logger.warn({module: "match-runner"}, "absence generation", e);
        }
    }
    ...
    if (userLineupJson) {
        try {
            ...
            for (const pos of ["GK", "DEF", "MID", "FWD"]) {
                ...
                for (let i = 0; i < need; i++) {
                    ...
                    if (pick) {
                        ...
                        usedRestIds.add(pick.id as string);
                    }
                }
            }
            ...
            const savedNames = pickedIds.map(id => {
                const p = rows.results.find(r => r.id === id);
    ...
    if (userLineupJson) {
        try {
            ...
            for (const p of picks) {
                if (p.matchPosition) matchPositionMap.set(p.playerId, p.matchPosition);
            }
        } catch (e) {
            logger.warn({module: "match-runner"}, "parse userLineupJson", e);
        }
    }
    ...
    if (matchPositionMap.size > 0) {
        ...
        for (const p of starters) {
            if (p.matchPosition) {
                if (p.matchPosition === "GK") hasGK = true;
                continue;
            }
        ...
        if (!hasGK && starters.length > 0) {
            ...
            for (const p of starters) {
                if (p.goalkeeping > best.goalkeeping) best = p;
            }
...
export async function copyOrCreateLineup(db: D1Database, teamId: string, calendarId: string): Promise<void> {
    // U21 týmy nemají human management — vždy rovnou auto-pick (user-saved lineup
    // by patřil k A-týmu se seniory, kopie by sem nepatřila).
    const team = await db.prepare("SELECT team_type FROM teams WHERE id = ?")
        .bind(teamId).first<{ team_type: string }>()
        .catch((e) => { logger.warn({ module: "match-runner" }, "team_type lookup", e); return null; });
    if (team?.team_type === "u21") {
...
export async function createAutoLineup(
    ...
    const savedLineup = await db.prepare("SELECT formation FROM lineups WHERE team_id = ? ORDER BY submitted_at DESC LIMIT 1")
        .bind(teamId).first<{ formation: string }>().catch((e) => {
            logger.warn({module: "match-runner"}, "query failed", e);
            return null;
        });
