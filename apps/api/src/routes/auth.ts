...
authRouter.post("/login", async (c) => {
  ...
  const team = await c.env.DB.prepare(
    `SELECT t.id, t.name, t.primary_color, t.secondary_color, t.badge_pattern, t.game_date, v.name as village_name, v.district, t.budget, t.league_id,
     ...
     COALESCE(t.season_start, (SELECT MIN(sc.scheduled_at) FROM season_calendar sc WHERE sc.league_id = t.league_id AND sc.season_number = (SELECT MAX(sc2.season_number) FROM season_calendar sc2 WHERE sc2.league_id = t.league_id))) as season_start,
     COALESCE(t.season_end, (SELECT MAX(sc.scheduled_at) FROM season_calendar sc WHERE sc.league_id = t.league_id AND sc.season_number = (SELECT MAX(sc2.season_number) FROM season_calendar sc2 WHERE sc2.league_id = t.league_id))) as season_end
