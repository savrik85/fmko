...
describe("tipy k naskladnění podle předpovědi", () => {
  ...
  it("v červenci na slunci je svařák poslední", () => {
    const h = concessionDemandHints("sunny", 7);
    ...
    for (const w of ["sunny", "cloudy", "rain", "wind", "snow"] as const) {
      for (let m = 1; m <= 12; m++) {
        for (const t of concessionDemandHints(w, m)) {
          expect(t.hint.length, `${w}/${m}/${t.key}`).toBeGreaterThan(3);
          expect(t.label.length, `${w}/${m}/${t.key}`).toBeGreaterThan(0);
  ...
  it("krajní situace dostanou výrazný popis, průměrné mírný", () => {
    const zima = concessionDemandHints("snow", 12).find((x) => x.key === "mulled_wine")!;
    const leto = concessionDemandHints("sunny", 7).find((x) => x.key === "mulled_wine")!;
    expect(zima.hint).not.toBe(leto.hint);
    const bezne = concessionDemandHints("cloudy", 5).find((x) => x.key === "beer")!;
