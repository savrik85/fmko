...
import { EQUIPMENT_ICONS, condBarColor, condColor, daysLeft, formatCZK, } from "./types";
/**
...
export default function EquipmentPageWrapper() {
    return (<Suspense fallback={<div className="page-container flex items-center justify-center min-h-[50vh]"><Spinner /></div>}>
      <EquipmentPage />
    </Suspense>);
}
function EquipmentPage() {
    ...
    const tab = searchParams.get("tab") === "bazar" ? "bazar" : "mine";
    const setTab = (next) => {
    ...
    const refresh = useCallback(async () => {
        ...
        const [eq, t] = await Promise.all([
            apiFetch(`/api/teams/${teamId}/equipment`),
    ...
    const refreshBazar = useCallback(async () => {
        ...
        try {
            setBazar(await apiFetch(`/api/teams/${teamId}/equipment-market`));
        }
        catch (e) {
    ...
    const handleRepair = async (category, label, condition, cost) => {
        if (!teamId || acting)
            return;
        const ok = await confirm({
            title: `Opravit ${label}?`,
    ...
    const handleList = async (category, price) => {
        if (!teamId || acting)
            return;
    ...
    const handlePawn = async (category) => {
        if (!teamId || acting || !selling)
            return;
    ...
    const handleWithdraw = async (listing) => {
        if (!teamId || acting)
            return;
        setActing("w-" + listing.id);
        if (await apiAction(apiFetch(`/api/teams/${teamId}/equipment-market/${listing.id}`, { method: "DELETE" }), "Stažení se nezdařilo"))
            await refreshAll();
        setActing(null);
    };
    const handleBuy = async (l) => {
        ...
        if (!ok)
            return;
        setActing("b-" + l.id);
        if (await apiAction(apiFetch(`/api/teams/${teamId}/equipment-market/${l.id}/buy`, { method: "POST" }), "Nákup se nezdařil"))
            await refreshAll();
        setActing(null);
    };
    if (loading)
    ...
    if (!data || !team)
        return <div className="page-container">Data nenalezena.</div>;
    ...
    return (<div className="page-container space-y-5">
      {confirmDialog}
      <SellDialog option={selling} busy={!!acting} onClose={() => setSelling(null)} onList={handleList} onPawn={handlePawn} onRepair={repairNow}/>

      {/* Tab bar */}
      <Tabs value={tab} onChange={setTab} ariaLabel="Vybavení" items={[
            { key: "mine", label: "Moje vybavení" },
            { key: "bazar", label: "Bazar", count: bazarCount || null },
        ]}/>

      {tab === "bazar" ? (<BazarTab data={bazar} loading={bazarLoading} budget={team.budget} busy={acting} onBuy={handleBuy} onWithdraw={handleWithdraw}/>) : (<>
          {/* ═══ Active effects ═══ */}
          ...
          {activeEffects.length > 0 && (<div className="card p-4 sm:p-5">
              <button type="button" onClick={() => setBonusesOpen((v) => !v)} aria-expanded={bonusesOpen} className="w-full flex items-center justify-between gap-2 sm:pointer-events-none">
              ...
              <div className={bonusesOpen ? "block" : "hidden sm:block"}>
                <div className="flex gap-3 flex-wrap">
                  {activeEffects.map((e) => (<div key={e.label} className="flex items-center gap-1.5 bg-pitch-50 text-pitch-700 px-3 py-1.5 rounded-soft">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {data.categories.map((cat) => {
                ...
                return (<div key={cat.key} className="card p-4">
                  {/* Header */}
                  <div className="flex items-center gap-2.5 mb-3">
                    ...
                    <div className="flex gap-1 shrink-0">
                      {[1, 2, 3].map((l) => (<div key={l} className={`w-3 h-3 rounded-full ${l <= cat.level ? "bg-pitch-400" : "bg-gray-200"}`}/>))}
                    </div>
                  </div>

                  {/* Condition bar */}
                  {cat.level > 0 && (<div className="mb-3">
                  ...
                  {/* Upgrade info + action */}
                  {upgrade && !upgrade.locked && (<div className="flex items-center justify-between gap-3 mt-1">
                      <div>
                      ...
                      <button onClick={() => handleUpgrade(upgrade)} disabled={!canUpgrade || !!acting} className={`${CARD_BTN} ${canUpgrade ? "bg-pitch-500 text-white hover:bg-pitch-600" : "bg-gray-100 text-gray-400 cursor-not-allowed"}`}>
                  ...
                  {!upgrade && cat.level === 3 && (<div className="text-xs text-pitch-600 font-heading font-bold mt-2">Maximální úroveň</div>)}

                  {/* Repair */}
                  {repair && (<div className="flex items-center justify-between gap-3 mt-2 pt-2 border-t border-gray-100">
                      ...
                      <button onClick={() => handleRepair(repair.category, repair.label, repair.condition, repair.cost)} disabled={!canRepair || !!acting} className={`${CARD_BTN} ${canRepair ? "bg-gold-500 text-white hover:bg-gold-600" : "bg-gray-100 text-gray-400 cursor-not-allowed"}`}>
                        {acting === "r-" + cat.key ? "..." : "Opravit"}
                      </button>
                    </div>)}

                  {/* Prodej — vystavené v bazaru vypadá jinak než dostupné k prodeji */}
                  {sell && (<div className="mt-2 pt-2 border-t border-gray-100">
                      {sell.listingId ? (<div className="flex items-center justify-between gap-3">
                          ...
                          <button onClick={() => handleWithdraw({ id: sell.listingId, category: sell.category, categoryLabel: sell.label, level: sell.level, price: sell.listedPrice ?? 0, condition: sell.condition, conditionAtListing: sell.condition, expiresAt: sell.listedUntil ?? "" })} disabled={!!acting} className={`${CARD_BTN} border border-gray-300 text-muted hover:text-ink hover:border-gray-400 disabled:opacity-50`}>
                            {acting === "w-" + sell.listingId ? "..." : "Stáhnout"}
                          </button>
                        </div>) : (<div className="flex items-center justify-between gap-3">
                          {/* Cenu v bazaru si určuje hráč, žádné číslo mu tu nepodsouváme.
                                Výkup zastavárny je naopak pevně daný, ten smysl má. */}
                          <div className="text-sm text-muted">
                            Prodej — zastavárna dá hned <span className="font-heading font-bold text-ink tabular-nums">{formatCZK(sell.pawnQuote)}</span>
                          </div>
                          <button onClick={() => setSelling(sell)} disabled={!!acting} className={`${CARD_BTN} border border-gray-300 text-muted hover:text-ink hover:border-gray-400 disabled:opacity-50`}>
                            Prodat
                          </button>
                        </div>)}
                    </div>)}
