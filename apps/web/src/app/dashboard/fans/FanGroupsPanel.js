...
function Pruh({ label, value, color, hint }) {
    return (<div>
...
function Akce({ group, teamId, onChanged, confirm }) {
    ...
    const proved = async (a, variant) => {
        ...
        setBusy(a.action);
        setVysledek(null);
        const res = await apiFetch(`/api/teams/${teamId}/fans/groups/${group.id}/action`, { method: "POST", body: JSON.stringify({ action: a.action, variant: variant?.key }) }).catch((e) => {
            console.error("akce s partou:", e);
            return { error: "Akci se nepodařilo provést." };
        });
        setBusy(null);
        if (res?.error) {
            showError("Nepovedlo se", res.error);
            return;
        }
    ...
    return (<div className="mt-3 space-y-2">
      {vysledek && (<p className="rounded-lg px-3 py-2 text-sm" style={{ background: "var(--color-paper)" }}>
          {vysledek}
        </p>)}
      {group.options.map((a) => (<div key={a.action}>
          {a.variants.length === 0 ? (<button className="btn btn-md btn-secondary w-full" disabled={!a.available || busy !== null} onClick={() => proved(a)}>
              {a.label}{a.cost > 0 ? ` · ${formatCZK(a.cost)}` : ""}
            </button>) : (<details className="rounded-lg" style={{ background: "var(--color-paper)" }}>
              ...
              <div className="px-3 pb-3 space-y-2">
                ...
                {a.variants.map((v) => (<button key={v.key} className="btn btn-md btn-secondary w-full" disabled={!a.available || busy !== null} onClick={() => proved(a, v)}>
                    {v.label}{v.cost > 0 ? ` · ${formatCZK(v.cost)}` : ""}
                  </button>))}
              </div>
            </details>)}
          {!a.available && a.blockedReason && (<p className="text-sm text-muted mt-1">{a.blockedReason}</p>)}
        </div>))}
    </div>);
}
function LeaderRadek({ leader }) {
...
export function FanGroupsPanel({ data, teamId, onChanged }) {
    ...
    return (<div className="space-y-5">
      ...
      {/* ═══ Party ═══ */}
      {data.groups.map((g) => (<div key={g.id} className="card p-4 sm:p-5">
          ...
          {g.sectorClosed && (<div className="mt-3 rounded-lg bg-card-red/10 px-3 py-2 text-sm">
              🚫 <strong>Sektor je uzavřený</strong> za trest ještě na{" "}
              {zavrenoNaZapasy(g.closedMatches)}. Na hřiště se nedostanou.
            </div>)}
          {g.ticketDiscount > 0 && (<div className="mt-3 rounded-lg bg-pitch-500/10 px-3 py-2 text-sm">
          ...
          {g.leader && (<div className="mt-4 pt-4 border-t border-gray-100">
      ...
      {data.recentEvents.length > 0 && (<div className="card p-4 sm:p-5">
          <SectionLabel>Co klub v poslední době potkalo</SectionLabel>
          <ul className="mt-2 space-y-2">
            {data.recentEvents.map((u, i) => (<li key={`${u.kind}-${u.gameDate}-${i}`} className="text-sm flex items-baseline gap-2">
      ...
      <div className="card p-4 sm:p-5">
        ...
        {data.recentIncidents.length === 0 ? (<p className="mt-2 text-sm text-muted">
            Zatím klid. Delegát nemá co zapsat a ty nemáš co platit.
          </p>) : (<>
            <ul className="mt-2 space-y-3">
              {data.recentIncidents.map((i) => (<li key={i.id} className="text-sm leading-relaxed">
                  <div className="flex items-baseline justify-between gap-3">
                    <span className="text-muted">
                      {formatDatum(i.gameDate)}
                      {i.minute != null && ` · ${i.minute}′`}
                    </span>
                    {i.fine > 0 && (<span className="font-semibold text-card-red shrink-0">−{formatCZK(i.fine)}</span>)}
                  </div>
                  <div>{i.text}</div>
                  {(i.sectorClosedMatches > 0 || i.fansLost > 0) && (<div className="text-muted">
                      {i.sectorClosedMatches > 0 && `Sektor zavřený na ${zavrenoNaZapasy(i.sectorClosedMatches)}. `}
                      {i.fansLost > 0 && odesloLidi(i.fansLost)}
                    </div>)}
                </li>))}
            </ul>
            {bordelCelkem > 0 && (<p className="mt-3 pt-3 border-t border-gray-100 text-sm">
...
//# sourceMappingURL=FanGroupsPanel.js.map