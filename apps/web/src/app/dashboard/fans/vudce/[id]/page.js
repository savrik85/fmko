 ...
 */
function osaColor(value, dobreVysoke) {
...
export default function VudceDetailPage() {
    ...
    if (!data)
        return <div className="page-container">Vůdce nenalezen.</div>;
    const l = data.leader;
    const s = sentimentWord(l.sentiment);
    return (<div className="page-container pb-24">
      <Link href="/dashboard/fans?tab=groups" className="text-sm text-muted hover:underline">← Fanoušci</Link>

      <div className="card p-4 mt-3">
        <div className="flex items-start gap-3">
          {l.avatar && (<FaceAvatar faceConfig={l.avatar} size={72} className="border border-ink/20 bg-white shrink-0 rounded-soft"/>)}
      ...
      {data.team && (<p className="text-sm text-muted mt-4">
          Chodí na{" "}
