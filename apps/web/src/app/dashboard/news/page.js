...
function ShareButton({ articleId }) {
    ...
    return (<button onClick={handleShare} className="inline-flex items-center gap-1 text-micro text-muted hover:text-pitch-500 font-heading font-bold transition-colors" title="Sdílet odkaz" type="button">
 ...
 */
function NovinoveOdstavce({ body }) {
    return (<>
      {body.split("\n").filter(Boolean).map((p, i) => (<p key={i} className={p.trimStart().startsWith("\u2022") ? "np-bod" : undefined}>
          {renderMarkdown(p)}
        </p>))}
    </>);
}
export default function NewsPage() {
    ...
    const [redakce, setRedakce] = useState([]);
    ...
    const loadClassifieds = () => {
        if (!teamId)
            return;
        apiFetch(`/api/teams/${teamId}/classifieds`)
            .then((data) => {
    ...
    useEffect(() => {
        if (!teamId)
            return;
        Promise.all([
            apiFetch(`/api/teams/${teamId}/news`),
    ...
    useEffect(() => {
        ...
        if (!ligaId)
            return;
        apiFetch(`/api/leagues/${ligaId}/journalists`)
    ...
    const submitAd = async () => {
        ...
        setAdError("");
        try {
            const res = await apiFetch(`/api/teams/${teamId}/classifieds`, {
                method: "POST",
            ...
            setAdError(e.message || "Chyba při vytváření inzerátu");
        }
    ...
    if (!team)
        return <div className="page-container">Tým nenalezen.</div>;
    ...
    const transferArticles = articles.filter((a) => ["transfer", "celebrity_arrival", "celebrity_signing"].includes(a.type));
    ...
    const latestInterviewWeek = interviewArticles.length > 0
        ? (interviewArticles[0].gameWeek ?? 0)
        : null;
    const currentWeekInterviews = latestInterviewWeek != null && latestInterviewWeek > 0
        ? interviewArticles.filter((a) => a.gameWeek === latestInterviewWeek)
        : interviewArticles;
    ...
    const latestPlayerIvWeek = playerInterviewArticles.length > 0
        ? (playerInterviewArticles[0].gameWeek ?? 0)
        : null;
    const currentPlayerInterviews = latestPlayerIvWeek != null && latestPlayerIvWeek > 0
        ? playerInterviewArticles.filter((a) => a.gameWeek === latestPlayerIvWeek)
    ...
    const secondaryStory = (gremiumArticles[0] && gremiumArticles[0].id !== leadStory?.id ? gremiumArticles[0] : null)
    ...
    const otherArticles = articles.filter((a) => !["match", "round_results", "round_summary", "standing", "ai_report", "matchday_preview", "promotion", "transfer", "celebrity_arrival", "celebrity_signing", "interview", "player_interview", "post_match_interview", "ultras_report", "governance_minutes"].includes(a.type)
    ...
    const vydani = articles.find((a) => a.gameWeek)?.gameWeek ?? null;
    const rocnik = ["I", "II", "III", "IV", "V", "VI", "VII", "VIII", "IX", "X"][season - 1] ?? String(season);
    return (<div className="page-container">

      {/* Výběr okresu — decentní, nad hlavičkou listu */}
      {allLeagues.length > 1 && (<div className="flex items-center gap-2 mb-3 text-sm">
          <span className="text-muted">Zobrazit zpravodaj:</span>
          <select value={selectedLeagueId ?? "own"} onChange={(e) => handleLeagueChange(e.target.value)} className="bg-transparent border-b border-ink/30 px-1 py-0.5 font-heading font-bold focus:outline-none focus:border-ink">
      ...
      <header className="np-dvojlinka pt-2">
        ...
        <h1 className="np-titulek text-center text-[2.1rem] sm:text-5xl my-1.5" style={{ fontVariant: "small-caps" }}>
          {newspaperName}
        </h1>
      ...
      {articles.length === 0 ? (<div className="text-center text-muted py-16 italic np-text">
          Redakce zatím nemá žádné zprávy. Odehraj první zápas a noviny se rozjedou!
        </div>) : (<div className="space-y-8">

          {/* ═══ Úvodník + placená propagace ═══ */}
          {leadStory && (<section className="grid grid-cols-1 lg:grid-cols-[1fr_260px] gap-7">
              ...
              {promotionArticles.length > 0 && (<aside className="lg:border-l lg:border-ink/15 lg:pl-6">
                  <div className="sticky top-4">
                    ...
                    <div className="divide-y divide-ink/10">
                      {promotionArticles.slice(0, 3).map((p) => (<div key={p.id} id={`news-${p.id}`} className="py-3 first:pt-0">
          ...
          {/* ═══ Druhý hlavní článek ═══ */}
          {secondaryStory && (<section id={`news-${secondaryStory.id}`} className="border-t border-ink/20 pt-6">
          ...
          {roundSummaryStory && (<section id={`news-${roundSummaryStory.id}`} className="border-t border-ink/20 pt-6">
          ...
          {/* ═══ Prales Ultras ═══ */}
          {ultrasReports.map((ur) => (<section key={ur.id} id={`news-${ur.id}`} className="border-t border-ink/20 pt-6">
              ...
              {ur.photos && ur.photos.length > 0 && (<div className="grid grid-cols-2 lg:grid-cols-3 gap-x-5 gap-y-4 mt-5">
                  {ur.photos.map((ph) => (<figure key={ph.teamId}>
                      <img src={`/kotel-foto?text=${encodeURIComponent(ph.ultrasText)}&bg=${encodeURIComponent(ph.bannerColor)}&fg=${encodeURIComponent(ph.textColor)}&p=${encodeURIComponent(ph.bannerColor)}&s=${encodeURIComponent(ph.textColor)}&lvl=${encodeURIComponent(String(ph.level))}&att=${encodeURIComponent(String(ph.attendance))}&team=${encodeURIComponent(ph.teamName)}&cap=${encodeURIComponent(ph.caption)}`} alt={`Kotel ${ph.teamName}`} width={1200} height={630} className="w-full h-auto border border-ink/25" loading="lazy"/>
          ...
          {[
                { klic: "po-zapase", datum: postMatchArticles[0]?.date, node: postMatchArticles.length > 0 && (<section className="border-t border-ink/20 pt-6">
              <Kicker>🗣️ {postMatchArticles.length === 1 ? "Po zápase" : "Ohlasy po zápase"}</Kicker>
              <div className="divide-y divide-ink/10">
                {postMatchArticles.map((iv) => {
                            let meta = {};
                            try {
                                meta = JSON.parse(iv.body);
                            }
                            catch (e) {
                                console.error("parse pozápasového rozhovoru:", e);
                                meta = {};
                            }
                            return (<Rozhovor key={iv.id} clanek={iv} jmeno={meta.managerName ?? "Trenér"} role="Trenér" tym={meta.teamName} avatar={meta.managerAvatar} jmenoHref={(meta.teamId ?? iv.teamId) ? `/dashboard/manager/${meta.teamId ?? iv.teamId}` : undefined} text={meta.article ?? iv.body} poznamka={meta.refereeName ? `Rozhodčí ${meta.refereeName}${meta.incidentText ? ", sporná situace v zápase" : ""}` : undefined} poznamkaHref={meta.refereeId ? `/dashboard/rozhodci/${meta.refereeId}` : undefined}/>);
                        })}
              </div>
            </section>) },
                { klic: "treneri", datum: currentWeekInterviews[0]?.date, node: currentWeekInterviews.length > 0 && (<section className="border-t border-ink/20 pt-6">
              ...
              <div className="divide-y divide-ink/10">
                {currentWeekInterviews.map((iv) => {
                            ...
                            try {
                                ...
                                console.error("parse rozhovoru trenéra:", e);
                                meta = {};
                            }
                            return (<Rozhovor key={iv.id} clanek={iv} jmeno={meta.managerName ?? "Trenér"} role="Trenér" tym={meta.teamName} avatar={meta.managerAvatar} jmenoHref={iv.teamId ? `/dashboard/manager/${iv.teamId}` : undefined} text={meta.article ?? iv.body} poznamka={meta.vztah ? `${meta.vztah.popis}${meta.vztah.dopad ? ` — ${meta.vztah.dopad}` : ""}` : undefined}/>);
                        })}
              </div>
            </section>) },
                { klic: "hraci", datum: currentPlayerInterviews[0]?.date, node: currentPlayerInterviews.length > 0 && (<section className="border-t border-ink/20 pt-6">
              ...
              <div className="divide-y divide-ink/10">
                {currentPlayerInterviews.map((iv) => {
                            ...
                            try {
                                ...
                                console.error("parse rozhovoru hráče:", e);
                                meta = {};
                            }
                            return (<Rozhovor key={iv.id} clanek={iv} jmeno={meta.playerName ?? "Hráč"} jmenoId={meta.playerId} role={meta.position} tym={meta.teamName} avatar={meta.playerAvatar} text={meta.article ?? iv.body} poznamka={meta.effectNote}/>);
                        })}
              </div>
            </section>) },
            ]
          ...
          {transferArticles.length > 0 && (<section className="border-t border-ink/20 pt-6">
              <Kicker>🤝 Přestupy a spekulace</Kicker>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-6">
                {transferArticles.slice(0, 9).map((t) => (<div key={t.id} id={`news-${t.id}`} className="py-3 border-b border-ink/10">
          ...
          {(roundArticles.length > 0 || miscArticles.length > 0) && (<div className="border-t border-ink/20 pt-6">

              <div className="space-y-7 min-w-0">
                {/* Výsledky kola */}
                {roundArticles.map((article) => (<section key={article.id}>
                ...
                {/* Drobné zprávy */}
                {miscArticles.length > 0 && (<section>
                    <Kicker>📰 Další zprávy</Kicker>
                    <div className="np-sloupce np-sloupce-2">
                      {miscArticles.map((article) => (<ArticleWrapper key={article.id} article={article}>
          ...
          {!isOtherLeague && <section className="np-dvojlinka pt-4">
            ...
            {classifieds.length > 0 ? (<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-6 mb-4">
                {classifieds.map((ad) => (<div key={ad.id} className="py-3 border-b border-ink/15 relative group">
                    ...
                    {ad.isOwn && (<button onClick={() => deleteAd(ad.id)} className="absolute top-2 right-0 w-5 h-5 text-card-red text-xs opacity-0 group-hover:opacity-100 transition-opacity" title="Smazat inzerát">&#10005;</button>)}
                  </div>))}
              </div>) : (<div className="text-center text-muted text-sm italic mb-4 py-3">
                Zatím žádné inzeráty. Buďte první!
              </div>)}

            {!showAdForm ? (<div className="text-center">
                <button onClick={() => setShowAdForm(true)} className="text-sm font-heading font-bold border border-ink/40 px-4 py-1.5 hover:bg-ink hover:text-paper transition-colors">
                  + Podat inzerát ({adCost} Kč)
                </button>
              </div>) : (<div className="max-w-lg mx-auto border border-ink/25 p-4">
                <div className="font-heading font-bold text-sm mb-3">Nový inzerát</div>

                <div className="flex gap-2 flex-wrap mb-3">
                  {categories.map((cat) => (<button key={cat.key} onClick={() => setAdCategory(cat.key)} className={`text-xs px-2.5 py-1 font-heading font-bold border transition-colors ${adCategory === cat.key ? "bg-ink text-paper border-ink" : "border-ink/25 text-muted hover:border-ink/50"}`}>
                      {cat.icon} {cat.label}
                    </button>))}
                </div>

                <textarea value={adMessage} onChange={(e) => setAdMessage(e.target.value)} placeholder="Např.: Hledáme brankáře do okresního přeboru. Kontakt: 777 123 456" className="w-full border border-ink/25 p-3 text-sm resize-none bg-transparent focus:outline-none focus:border-ink transition-colors" rows={3} maxLength={200}/>
                <div className="flex items-center justify-between mt-2">
                ...
                <div className="flex gap-2 mt-3">
                  <button onClick={() => { setShowAdForm(false); setAdError(""); }} className="flex-1 text-sm py-2 text-muted hover:text-ink transition-colors">
                    Zrušit
                  </button>
                  <button onClick={submitAd} disabled={adSubmitting || adMessage.trim().length < 5} className="flex-1 text-sm py-2 font-heading font-bold text-paper bg-ink hover:bg-ink-light disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
          ...
          {redakce.length > 0 && (<section className="border-t border-ink/20 pt-6 mt-6">
              {/* Bez skloňování názvu listu — „Redakce Pražský Zpravodaju" nedopadlo dobře. */}
              <Kicker>✒️ Redakce</Kicker>
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                {redakce.map((r) => (<Link key={r.id} href={`/dashboard/redakce/${r.id}`} className="flex items-start gap-2 group">
                    {r.avatar && (<FaceAvatar faceConfig={r.avatar} size={44} className="border border-ink/30 bg-white shrink-0"/>)}
 ...
 */
function Podpis({ clanek, stred = false }) {
    const autor = clanek.journalist;
    return (<div className={`flex items-center gap-2 text-micro text-muted ${stred ? "justify-center" : ""}`}>
      {autor ? (<Link href={`/dashboard/redakce/${autor.id}`} className="uppercase tracking-[0.12em] font-heading font-bold text-ink hover:underline underline-offset-2" title={`Profil redaktora — ${STYL_POPIS[autor.style] ?? "redakce"}`}>
          {autor.name}
        </Link>) : (<span className="uppercase tracking-[0.15em] font-heading font-bold">Redakce</span>)}
 ...
 */
function vytahnoutCitat(odstavce) {
    const kandidati = [];
    for (const odst of odstavce) {
        for (const m of odst.matchAll(/„([^„""]+)[""]/g))
            kandidati.push(m[1]);
        const casti = odst.split('"');
        for (let i = 1; i < casti.length; i += 2)
            kandidati.push(casti[i]);
    }
    return kandidati
        .map((c) => c.trim())
 ...
 */
function Rozhovor({ clanek, jmeno, jmenoId, jmenoHref, role, tym, avatar, text, poznamka, poznamkaHref, }) {
    ...
    return (<article id={`news-${clanek.id}`} className="py-6 first:pt-0">
      ...
      <div className="np-text np-sloupce np-sloupce-2 text-[15px]">
        {maAvatar && (<figure className="np-portret">
            ...
            <figcaption className="text-micro leading-tight mt-1 not-italic border-t border-ink/20 pt-1">
              {jmenoId ? (<EntityLink type="player" id={jmenoId} className="font-heading font-bold text-ink">{jmeno}</EntityLink>) : jmenoHref ? (<Link href={jmenoHref} className="entity-link font-heading font-bold text-ink">{jmeno}</Link>) : (<span className="font-heading font-bold text-ink">{jmeno}</span>)}
              {(role || tym) && (<span className="block text-muted">{[role, tym].filter(Boolean).join(" · ")}</span>)}
            </figcaption>
          </figure>)}

        {odstavce.map((p, i) => (<Fragment key={i}>
            <p className={i === 0 && maPerex ? "np-perex" : undefined}>{p}</p>
            {citat && i === 0 && <blockquote className="np-citat">„{citat}“</blockquote>}
          </Fragment>))}
      </div>

      {poznamka && (<div className="mt-3">
          <span className="text-micro font-heading font-bold uppercase tracking-wider text-muted border-l-2 border-ink/30 pl-2">
            🧱 {poznamkaHref ? (<Link href={poznamkaHref} className="entity-link">{poznamka}</Link>) : poznamka}
          </span>
        </div>)}
    </article>);
}
...
function RoundResults({ body }) {
    ...
    return (<div className="sm:columns-2 sm:gap-x-8">
      {structured.map((r, i) => {
            const [h, a] = r.score.split(":").map(Number);
            const isDraw = h === a;
            const homeWin = h > a;
            return (<div key={i} className="flex items-baseline text-sm py-1.5 border-b border-ink/10 break-inside-avoid-column">
