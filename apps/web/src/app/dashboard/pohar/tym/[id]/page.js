...
function strengthColor(s) {
    if (s >= 55)
        return "bg-card-red/10 text-card-red";
    if (s >= 42)
...
function strengthLabel(s) {
    if (s >= 60)
        return "velmi silný soupeř";
    if (s >= 50)
        return "silný soupeř";
    if (s >= 40)
        return "vyrovnaný soupeř";
    if (s >= 25)
        return "slabší soupeř";
...
export default function CupTeamDetailPage() {
    ...
    useEffect(() => {
        if (!teamId || !params.id)
            return;
        apiFetch(`/api/teams/${teamId}/cup/team/${params.id}`)
            .then(setData)
    ...
    if (!data?.team)
        return <div className="page-container py-10 text-center text-muted">Pohárový tým nenalezen.</div>;
    const t = data.team;
    return (<div className="page-container py-6 space-y-6">
      <button onClick={() => router.back()} className="text-sm text-muted hover:text-ink">← Zpět na pohár</button>

      {/* Hlavička týmu */}
      <div className="card p-5 flex items-center gap-4">
        <span className={`w-14 h-14 rounded-full flex items-center justify-center text-lg font-heading font-bold shrink-0 border ${bestTextOn(t.color || "#9aa18c") === "light" ? "text-white border-transparent" : "text-gray-900 border-gray-300"}`} style={{ background: t.color || "#9aa18c" }}>
          {initials(t.name)}
        </span>
      ...
      {/* Cesta pohárem */}
      {data.matches.length > 0 && (<div>
          <SectionLabel>Cesta pohárem</SectionLabel>
          <div className="card mt-2">
            {data.matches.map((m, i) => {
                const sim = m.status === "simulated" && m.homeScore != null;
                return (<div key={i} className="flex items-center gap-3 px-4 py-2.5 border-b border-gray-50 last:border-b-0">
      ...
      <div>
        ...
        {data.squad.length > 0 ? (<div className="card mt-2 overflow-x-auto table-scroll">
            <table className="w-full text-sm">
              ...
              <tbody>
                {data.squad.map((p, i) => (<tr key={i} className="border-t border-gray-50">
          ...
          </div>) : (<div className="card mt-2 p-4 text-sm text-muted">
            {t.isBig
