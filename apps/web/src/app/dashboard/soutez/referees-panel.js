...
export function RefereesPanel({ data, state, teamId, myOpen, jeKomisar, onChanged }) {
    ...
    if (!data)
        return <Empty>Načítám listinu rozhodčích…</Empty>;
    ...
    return (<div className="space-y-4">
      <div className="card p-5">
        ...
        {!data.canBan && data.banReason && (<div className="text-sm mt-2" style={{ color: "#A32B1F" }}>{data.banReason}</div>)}
      </div>

      {data.round && data.delegation.length > 0 && (<div className="card p-5">
          ...
          <div className="divide-y mt-3" style={{ borderColor: "var(--color-line)" }}>
            {data.delegation.map((m) => (<MatchRow key={m.matchId} m={m} teamId={teamId} muzeMenit={!!muzeObsazovat && (data.swapsUsed < data.maxSwaps || m.swapped)} onSwap={() => setSwapTarget(m)}/>))}
          </div>

          {muzeObsazovat && data.swapsUsed >= data.maxSwaps && (<p className="text-sm mt-3" style={{ color: "#A32B1F" }}>
      ...
      {flagged.length > 0 && (<div className="card p-5" style={{ background: "#FBEAE8" }}>
          ...
          <div className="mt-2 space-y-1">
            {flagged.map((r) => (<div key={r.refereeId} className="text-sm font-semibold">{r.name}</div>))}
          </div>
        </div>)}
      ...
      <div className="card p-5">
          ...
          <div className="divide-y" style={{ borderColor: "var(--color-line)" }}>
            {data.referees.map((r) => (<div key={r.refereeId} className="py-2.5">
                <div className="flex items-center justify-between gap-3">
                  <div className="min-w-0">
                    <div className="text-base font-semibold break-words">
                      {r.name}
                      {r.banned && <span className="text-sm text-danger"> · vyškrtnut</span>}
                      {r.paused && !r.banned && (<span className="text-sm" style={{ color: "#A32B1F" }}>
                          {" "}· stopka do {r.pausedUntil}. kola
                        </span>)}
                      {r.flagged && !r.banned && !r.paused && (<span className="text-sm" style={{ color: "#A32B1F" }}> · pod drobnohledem</span>)}
                    </div>
                ...
                {state.enabled && teamId && !r.banned && (<div className="flex flex-wrap gap-2 mt-2">
                    {jeKomisar && !r.paused && data.canPause && (<button className="btn btn-md btn-secondary" onClick={() => setPauseTarget(r)}>
                        Stopka na {data.pauseWeeks} kola
                      </button>)}
                    {/* Komisař rozhoduje sám, takže ho vlastní otevřený návrh nebrzdí. */}
                    {data.canBan && (!myOpen || jeKomisar) && (<button className="btn btn-md btn-secondary" onClick={() => setBanTarget(r)}>
                        {jeKomisar ? "Vyškrtnout" : "Navrhnout vyškrtnutí"}
                      </button>)}
                  </div>)}
              </div>))}
          </div>
        {jeKomisar && !data.canPause && data.pauseReason && (<div className="text-sm mt-3" style={{ color: "#A32B1F" }}>{data.pauseReason}</div>)}
      </div>

      {banTarget && teamId && (<BanForm referee={banTarget} teamId={teamId} deposit={state.deposit} jeKomisar={jeKomisar} onClose={() => setBanTarget(null)} onSaved={() => { setBanTarget(null); onChanged(); }}/>)}

      {swapTarget && teamId && (<SwapForm match={swapTarget} teamId={teamId} volni={data.freeReferees} gameWeek={data.round?.gameWeek ?? 0} onClose={() => setSwapTarget(null)} onSaved={() => { setSwapTarget(null); onChanged(); }}/>)}

      {pauseTarget && teamId && data.round && (<PauseForm referee={pauseTarget} teamId={teamId} weeks={data.pauseWeeks} fromWeek={data.round.gameWeek} maxPauses={data.maxPauses} onClose={() => setPauseTarget(null)} onSaved={() => { setPauseTarget(null); onChanged(); }}/>)}
    </div>);
}
...
function PauseForm({ referee, teamId, weeks, fromWeek, maxPauses, onClose, onSaved }) {
    ...
    const submit = async () => {
        if (kratkyDuvod)
            return;
        setSaving(true);
        const ok = await apiAction(apiFetch(`/api/teams/${teamId}/competition/referee-pauses`, {
    ...
    return (<Modal isOpen onClose={onClose} title="Stopka rozhodčímu" zavritKlikemVedle={false}>
      <div className="p-5 space-y-4">
        ...
        <div>
          ...
          <textarea className="input w-full mt-1" rows={3} maxLength={200} value={note} onChange={(e) => setNote(e.target.value)} placeholder="Proč ho na tři kola odstavuješ?"/>
          {kratkyDuvod && (<p className="text-sm text-red-700 mt-1">
...
function BanForm({ referee, teamId, deposit, jeKomisar, onClose, onSaved }) {
    ...
    const submit = async () => {
        if (kratkyDuvod)
            return;
        setSaving(true);
        const ok = await apiAction(apiFetch(primo
    ...
    return (<Modal isOpen onClose={onClose} title="Vyškrtnutí rozhodčího" zavritKlikemVedle={false}>
      <div className="p-5 space-y-4">
        ...
        <div>
          ...
          <textarea className="input w-full mt-1" rows={3} maxLength={200} value={note} onChange={(e) => setNote(e.target.value)} placeholder="Proč ho na listině dál nechceš?"/>
          {kratkyDuvod && (<p className="text-sm text-red-700 mt-1">
              Napiš aspoň {MIN_DUVOD} znaků. Za rozhodnutím komisaře musí být důvod.
            </p>)}
        </div>

        {jeKomisar && (<label className="flex items-start gap-2 text-sm">
...
function MatchRow({ m, teamId, muzeMenit, onSwap }) {
    ...
    return (<div className="py-2.5">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <div className="text-sm text-muted break-words">
            ...
            {m.swapped && (<span style={{ color: "#866D1E" }}> · vyměněno komisařem</span>)}
          </div>
          {m.swapReason && (<div className="text-sm text-muted break-words italic">„{m.swapReason}"</div>)}
        </div>
      ...
      {muzeMenit && (<button className="btn btn-md btn-secondary mt-2" onClick={onSwap}>
...
function SwapForm({ match, teamId, volni, gameWeek, onClose, onSaved }) {
    ...
    const kratkyDuvod = note.trim().length < MIN_DUVOD;
    const submit = async () => {
        if (kratkyDuvod || !refereeId)
            return;
        setSaving(true);
        const ok = await apiAction(apiFetch(`/api/teams/${teamId}/competition/referee-swap`, {
            method: "POST", headers: { "Content-Type": "application/json" },
    ...
    return (<Modal isOpen onClose={onClose} title="Výměna rozhodčího" zavritKlikemVedle={false}>
      <div className="p-5 space-y-4">
        <div className="text-lg font-heading">
        ...
        {mujZapas && (<div className="rounded-lg p-3 text-sm" style={{ background: "#FBEAE8" }}>
        ...
        <div>
          ...
          {serazeni.length === 0 ? (<p className="text-sm mt-1" style={{ color: "#A32B1F" }}>
              Ten den nemá nikdo z listiny volno. Vyměnit se nedá — všichni už někde pískají.
            </p>) : (<select className="select w-full mt-1" value={refereeId} onChange={(e) => setRefereeId(e.target.value)}>
              <option value="">— vyber rozhodčího —</option>
              {serazeni.map((r) => (<option key={r.refereeId} value={r.refereeId}>
        ...
        <div>
          ...
          <textarea className="input w-full mt-1" rows={3} maxLength={200} value={note} onChange={(e) => setNote(e.target.value)} placeholder="Proč tenhle zápas přeobsazuješ?"/>
          {kratkyDuvod && (<p className="text-sm text-red-700 mt-1">
              Napiš aspoň {MIN_DUVOD} znaků. Za rozhodnutím komisaře musí být důvod.
            </p>)}
        </div>
        ...
        <div className="flex gap-2">
          <button className="btn btn-lg btn-secondary flex-1" onClick={onClose}>Zrušit</button>
          <button className="btn btn-lg btn-primary flex-1" disabled={saving || kratkyDuvod || !refereeId} onClick={submit}>
...
//# sourceMappingURL=referees-panel.js.map