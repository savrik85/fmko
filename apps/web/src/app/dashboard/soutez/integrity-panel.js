...
const kurz = (x) => (x / 100).toFixed(2).replace(".", ",");
const STAV = {
...
export function IntegrityPanel({ state, teamId, jsemKomisar, onChanged }) {
    ...
    const [zabavit, setZabavit] = useState(null);
    ...
    useEffect(() => {
        if (!teamId)
            return;
        apiFetch(`/api/teams/${teamId}/betting/book`)
            .then(setData)
            .catch((e) => {
    ...
    if (!data)
        return <Empty>Načítám knihu sázek…</Empty>;
    const podezrele = data.transfers.filter((t) => t.priznaky.length > 0);
    const provedZabaveni = async () => {
        if (!zabavit || !teamId)
            return;
        setOdesilam(true);
        const ok = await apiAction(apiFetch(`/api/teams/${teamId}/betting/tickets/${zabavit.ticketId}/confiscate`, {
    ...
    const provedZakaz = async () => {
        if (!teamId || !cil)
            return;
        setOdesilam(true);
        const ok = await apiAction(apiFetch(`/api/teams/${teamId}/betting/bans`, {
            method: "POST", headers: { "Content-Type": "application/json" },
    ...
    return (<div className="space-y-4">
      ...
      <Rozbaleni title="Přestupy soutěže" right={`${data.transfers.length} ${plural(data.transfers.length, "obchod", "obchody", "obchodů")}`} note={data.transfers.length === 0
        ...
        {data.transfers.length > 0 && (<>
            <ul className="divide-y divide-gray-50">
              {data.transfers.map((t, i) => (<li key={i} className="py-2.5">
                  ...
                  {t.priznaky.map((p, j) => (<div key={j} className="text-sm mt-1 leading-snug" style={{ color: "#A32B1F" }}>
                      ⚠ {p}
                    </div>))}
                </li>))}
            </ul>
      ...
      {/* ── Kniha sázek ──────────────────────────────────────────────────── */}
      <Rozbaleni title="Kniha sázek" right={`${data.tickets.length} ${plural(data.tickets.length, "tiket", "tikety", "tiketů")}`} note={data.tickets.length === 0
        ...
        {data.tickets.length > 0 && (<ul className="divide-y divide-gray-50">
            {data.tickets.map((t) => {
                const s = STAV[t.status] ?? STAV.open;
                return (<li key={t.ticketId} className="py-2.5">
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className="text-micro font-heading font-bold uppercase tracking-wide px-2 py-0.5 rounded-full" style={{ background: s.bg, color: s.fg }}>
                      {s.label}
                    </span>
                    ...
                    {t.gameWeek != null && (<span className="ml-auto text-micro text-muted tabular-nums">{t.gameWeek}. kolo</span>)}
                  </div>
                  ...
                  <div className="flex items-center gap-3 mt-1 flex-wrap text-sm tabular-nums">
                    ...
                    {jsemKomisar && t.status === "won" && t.teamId !== teamId && (<button type="button" onClick={() => { setZabavit(t); setDuvod(""); }} className="ml-auto text-sm font-heading font-bold text-card-red cursor-pointer">
                        Zabavit výhru
                      </button>)}
                  </div>
                </li>);
      ...
      {jsemKomisar && (<button type="button" onClick={() => { setZablokovat(true); setDuvod(""); setCil(""); }} className="w-full min-h-12 rounded-soft bg-pitch-500 text-white font-heading font-bold text-sm cursor-pointer">
      ...
      <Modal isOpen={!!zabavit} onClose={() => setZabavit(null)} title="Zabavit výhru" zavritKlikemVedle={false}>
        <div className="p-4 space-y-3">
          ...
          <div>
            ...
            <textarea id="duvod-zabaveni" value={duvod} onChange={(e) => setDuvod(e.target.value)} rows={3} maxLength={300} className="input w-full mt-1 text-sm" placeholder="Aspoň větu, klub i grémium to uvidí."/>
          </div>
          <button type="button" onClick={provedZabaveni} disabled={odesilam || duvod.trim().length < 10} className="w-full min-h-12 rounded-soft bg-card-red text-white font-heading font-bold text-sm disabled:opacity-50 cursor-pointer">
      ...
      <Modal isOpen={zablokovat} onClose={() => setZablokovat(false)} title="Zakázat klubu sázení" zavritKlikemVedle={false}>
        <div className="p-4 space-y-3">
          ...
          <div>
            ...
            <select id="cil-zakazu" value={cil} onChange={(e) => setCil(e.target.value)} className="select w-full mt-1 text-sm">
          ...
          <div>
            ...
            <input id="pocet-kol" type="number" min={1} max={30} value={kol} onChange={(e) => setKol(Math.max(1, Math.min(30, Number(e.target.value) || 1)))} className="input w-full mt-1 text-sm tabular-nums"/>
          </div>
          <div>
            <label htmlFor="duvod-zakazu" className="text-micro font-heading font-bold uppercase tracking-wide text-muted">
              Za co
            </label>
            <textarea id="duvod-zakazu" value={duvod} onChange={(e) => setDuvod(e.target.value)} rows={3} maxLength={300} className="input w-full mt-1 text-sm" placeholder="Aspoň větu, klub i grémium to uvidí."/>
          </div>
          <button type="button" onClick={provedZakaz} disabled={odesilam || !cil || duvod.trim().length < 10} className="w-full min-h-12 rounded-soft bg-pitch-500 text-white font-heading font-bold text-sm disabled:opacity-50 cursor-pointer">
...
//# sourceMappingURL=integrity-panel.js.map