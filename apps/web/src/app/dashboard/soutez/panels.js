...
import { Empty, GOLD, GOLD_SOFT, OpenProposalNote, Ornament, PersonLine, Portrait, Row, czk, formatDate, plural, signed, } from "./ui";
const LEDGER_LABEL = {
...
export function PokladnaPanel({ state, ledger }) {
    ...
    return (<div className="space-y-4">
      ...
      {zmeny.length > 0 && (<div className="card p-5" style={{ background: "rgba(196,160,53,0.10)" }}>
          <Ornament>Od příští sezóny</Ornament>
          <p className="text-sm text-muted mb-2">
            Tohle už kluby odhlasovaly. Rozpočet s tím počítá, i když to zatím neplatí.
          </p>
          {zmeny.map((z) => (<Row key={z.label} label={z.label} value={`${z.ted} → ${z.pak}`}/>))}
        </div>)}
      ...
      <div className="card p-5">
        ...
        <Row label="Transfery mezi kluby stejného majitele" value={state.rules.ban_own_owner_transfers ? "zakázané" : "povolené"}/>
        <Row label="Minimální stav hřiště" value={state.rules.min_pitch_condition > 0 ? `${state.rules.min_pitch_condition} ze 100` : "nehlídá se"}/>
        <Row label="Nejméně hráčů na soupisce" value={state.rules.squad_min > 0 ? String(state.rules.squad_min) : "bez omezení"}/>
        <Row label="Nejvíc hráčů na soupisce" value={state.rules.squad_max > 0 ? String(state.rules.squad_max) : "bez omezení"}/>
      </div>

      {ledger && ledger.summary.length > 0 && (<div className="card p-5">
          <Ornament>Sezóna podle položek</Ornament>
          {ledger.summary.map((s) => (<Row key={s.type} label={LEDGER_LABEL[s.type] ?? s.type} value={signed(s.total)}/>))}
        </div>)}

      {ledger && (<div className="card p-5">
          ...
          <div className="divide-y" style={{ borderColor: "var(--color-line)" }}>
            {ledger.entries.map((e) => (<div key={e.id} className="flex items-start justify-between gap-3 py-2">
...
export function VedeniPanel({ state, elections, avatars, board, teamId, onChanged, onPost }) {
    ...
    const [suspendRole, setSuspendRole] = useState(null);
    const [resignRole, setResignRole] = useState(null);
    /** Prezident je nadřízený ostatním předsedům — pozastavení vidí jen on. */
    const jsemPrezident = !!teamId && state.presidentTeamId === teamId;
    const candidate = async (id, on) => {
        if (!teamId)
            return;
        setBusy(id);
        const ok = await apiAction(apiFetch(`/api/teams/${teamId}/competition/elections/${id}/candidacy`, { method: on ? "POST" : "DELETE" }), on ? "Kandidaturu se nepodařilo podat" : "Kandidaturu se nepodařilo stáhnout");
        setBusy(null);
        if (ok)
            onChanged();
    };
    const vote = async (id, candidateTeamId) => {
        if (!teamId)
            return;
        setBusy(id);
        const ok = await apiAction(apiFetch(`/api/teams/${teamId}/competition/elections/${id}/vote`, {
    ...
    return (<div className="space-y-4">
      <div className="card p-5">
        ...
        <div className="space-y-3">
          {(state.roles ?? []).map((r) => {
            ...
            return (<div key={r.role} className="space-y-2">
                <div className="flex items-center gap-3">
                <Portrait avatar={r.holder ? avatars[r.holder.teamId] : null} name={r.holder?.managerName ?? r.holder?.teamName} size={48} ring={r.holder ? GOLD : "var(--color-muted-light)"}/>
                <div className="min-w-0 flex-1">
                  <div className="section-label mb-0" style={{ fontSize: "0.6875rem" }}>{r.label}</div>
                  {r.holder ? (<PersonLine managerName={r.holder.managerName} teamId={r.holder.teamId} teamName={r.holder.teamName} note={suspended ? <span className="text-danger">pravomoc pozastavena</span> : undefined}/>) : (<div className="text-base text-muted">
                      neobsazeno{jsemPrezident ? ", zastupuješ ji" : ""}
                    </div>)}
                </div>
                {jsemPrezident && r.holder && r.role !== "predseda" && !suspended && (<button className="btn btn-md btn-secondary shrink-0" onClick={() => setSuspendRole({ role: r.role, label: r.label })}>
                    Pozastavit
                  </button>)}
                {r.holder?.teamId === teamId && (<button className="btn btn-md btn-secondary shrink-0" onClick={() => setResignRole({ role: r.role, label: r.label })}>
                    Rezignovat
                  </button>)}
                </div>
                ...
                <div className="rounded-lg p-3 text-sm" style={{ background: "var(--color-paper)" }}>
                  ...
                  <ul className="mt-0.5 space-y-1">
                    {r.powers.map((v, i) => (<li key={i} className="text-muted flex gap-2">
        ...
        {jsemPrezident && (<div className="mt-4 rounded-lg p-3 text-sm" style={{ background: "rgba(196,160,53,0.14)" }}>
      ...
      {open.length > 0 && (<div className="card p-5 space-y-5">
          ...
          <div className="rounded-lg p-4" style={{ background: "rgba(196,160,53,0.10)" }}>
            ...
            ].map((v, i) => (<li key={i} className="flex gap-2">
                  <span aria-hidden="true" style={{ color: GOLD }}>◆</span>
                  <span className="min-w-0">{v}</span>
                </li>))}
            </ul>
          ...
          {open.map((e) => {
                ...
                return (<div key={e.id} className="rounded-lg overflow-hidden" style={{ background: "var(--color-paper)", boxShadow: `inset 0 0 0 1px ${GOLD_SOFT}` }}>
                ...
                <div className="space-y-2">
                  {e.candidates.map((k) => {
                        ...
                        return (<div key={k.teamId} className="flex items-center gap-3 rounded-lg p-3" style={{
                                background: mine ? "rgba(196,160,53,0.14)" : "var(--color-surface)",
                                boxShadow: mine ? `inset 0 0 0 1px ${GOLD}` : "inset 0 0 0 1px var(--color-line)",
                            }}>
                        <Portrait avatar={avatars[k.teamId]} name={k.managerName ?? k.teamName} size={40} ring={mine ? GOLD : "var(--color-muted-light)"}/>
                        <div className="min-w-0 flex-1">
                          <PersonLine managerName={k.managerName} teamId={k.teamId} teamName={k.teamName}/>
                        </div>
                        <button className={`btn btn-md shrink-0 ${mine ? "btn-primary" : "btn-secondary"}`} style={{ minWidth: 108 }} disabled={busy === e.id} onClick={() => vote(e.id, k.teamId)}>
                ...
                <button className={`btn btn-lg w-full mt-3 ${jsemKandidat ? "btn-ghost" : "btn-secondary"}`} disabled={busy === e.id} onClick={() => candidate(e.id, !jsemKandidat)}>
      ...
      {suspendRole && teamId && (<SuspendForm role={suspendRole} teamId={teamId} onClose={() => setSuspendRole(null)} onSaved={() => { setSuspendRole(null); onChanged(); }}/>)}

      {resignRole && teamId && (<ResignForm role={resignRole} teamId={teamId} hrozbaOdvolani={state.officials.some((o) => o.role === resignRole.role && o.status === "suspended")} onClose={() => setResignRole(null)} onSaved={() => { setResignRole(null); onChanged(); }}/>)}

      {board?.seat && (<Vlakno title="Kabinet" right={board.seat.roleLabel} subtitle="Neveřejné vlákno vedení soutěže. Kluby ho nevidí a v zápisu ze zasedání se neobjeví." messages={board.messages} maxLength={board.maxLength} canPost placeholder="Napiš ostatním do vedení…" teamId={teamId} avatars={avatars} onPost={(t) => onPost(t, "kabinet")}/>)}

      {board && (<Vlakno title="Stížnosti a vzkazy" subtitle="Veřejná nástěnka soutěže. Čte ji každý klub včetně toho, kterého se stížnost týká." messages={board.publicMessages} maxLength={board.maxLength} canPost={board.canPost} placeholder="Co ti v soutěži vadí? Uvidí to všichni…" teamId={teamId} avatars={avatars} onPost={(t) => onPost(t, "verejne")}/>)}

      {done.length > 0 && (<div className="card p-5">
          <Ornament>Výsledky voleb</Ornament>
          <div className="space-y-3">
            {done.map((e) => (<div key={e.id} className="flex items-center gap-3">
                <Portrait avatar={e.winnerTeamId ? avatars[e.winnerTeamId] : null} name={e.winnerManager ?? e.winnerName} size={40} ring={e.winnerTeamId ? GOLD : "var(--color-muted-light)"}/>
                <div className="min-w-0 flex-1">
                  <div className="section-label mb-0" style={{ fontSize: "0.6875rem" }}>{e.roleLabel}</div>
                  {e.winnerTeamId ? (<PersonLine managerName={e.winnerManager} teamId={e.winnerTeamId} teamName={e.winnerName ?? "?"} note={e.resultNote ?? undefined}/>) : (<div className="text-base text-muted">neobsazeno — {e.resultNote}</div>)}
                </div>
              </div>))}
          </div>
        </div>)}
...
function SuspendForm({ role, teamId, onClose, onSaved }) {
    ...
    const submit = async () => {
        setSaving(true);
        const ok = await apiAction(apiFetch(`/api/teams/${teamId}/competition/officials/${role.role}/suspend`, {
            method: "POST", headers: { "Content-Type": "application/json" },
    ...
    return (<Modal isOpen onClose={onClose} title="Pozastavit pravomoc" zavritKlikemVedle={false}>
      <div className="p-5 space-y-4">
        ...
        <div>
          ...
          <textarea className="input w-full mt-1" rows={3} maxLength={300} value={reason} onChange={(e) => setReason(e.target.value)} placeholder="Uvidí to celé grémium i on sám."/>
        </div>

        <div className="flex gap-2">
          <button className="btn btn-lg btn-secondary flex-1" onClick={onClose}>Zrušit</button>
          <button className="btn btn-lg btn-primary flex-1" disabled={saving || reason.trim().length < 10} onClick={submit}>
...
function ResignForm({ role, teamId, hrozbaOdvolani, onClose, onSaved }) {
    ...
    const submit = async () => {
        setSaving(true);
        const ok = await apiAction(apiFetch(`/api/teams/${teamId}/competition/officials/${role.role}/resign`, { method: "POST" }), "Demisi se nepodařilo podat");
        setSaving(false);
        if (ok)
            onSaved();
    };
    return (<Modal isOpen onClose={onClose} title="Rezignovat na funkci" zavritKlikemVedle={false}>
      <div className="p-5 space-y-4">
        ...
        <div className="rounded-lg p-3 text-sm" style={{ background: "#FBEAE8" }}>
          ...
          {hrozbaOdvolani && (<div className="mt-2 font-semibold">
...
export function GrantsPanel({ data, teamId, myOpen, onChanged }) {
    ...
    return (<div className="card p-5">
      ...
      {myOpen ? (<div className="mt-3">
          <OpenProposalNote p={myOpen}/>
        </div>) : data.freeBalance < data.min ? (<p className="text-sm mt-3" style={{ color: "#A32B1F" }}>
          Volných peněz je {czk(data.freeBalance)} — na dotaci to nestačí.
        </p>) : (<button className="btn btn-lg btn-primary w-full mt-3" onClick={() => setOpen(true)}>
          Navrhnout dotaci
        </button>)}

      {open && (<GrantForm data={data} teamId={teamId} onClose={() => setOpen(false)} onSaved={() => { setOpen(false); onChanged(); }}/>)}
    </div>);
}
function GrantForm({ data, teamId, onClose, onSaved }) {
    ...
    const cile = useMemo(() => (kind === "loan" ? data.teams.filter((t) => t.vMinusu) : data.teams), [kind, data.teams]);
    useEffect(() => {
        setTarget(cile[0]?.teamId ?? "");
    }, [cile]);
    const submit = async () => {
        setSaving(true);
        const ok = await apiAction(apiFetch(`/api/teams/${teamId}/competition/grants`, {
            method: "POST", headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
    ...
    return (<Modal isOpen onClose={onClose} title="Návrh na dotaci" zavritKlikemVedle={false}>
      <div className="p-5 space-y-4">
        ...
        <div>
          ...
          {kind === "pitch" && (<div className="text-sm text-muted mt-1">
              {podHranici === 0
          ...
          {kind === "equipment" && (<div className="text-sm text-muted mt-1">
              Rozdělí se rovným dílem mezi všechny kluby soutěže.
            </div>)}
          {spec?.seasonEnd && (<div className="text-sm text-muted mt-1">
        ...
        {spec?.targeted && (<div>
            ...
            {cile.length === 0 ? (<div className="text-sm mt-1" style={{ color: "#A32B1F" }}>
                Žádný klub soutěže není v mínusu. Půjčka je pro kluby v nouzi.
              </div>) : (<select className="select w-full mt-1" value={target} onChange={(e) => setTarget(e.target.value)}>
                {cile.map((t) => (<option key={t.teamId} value={t.teamId}>
        ...
        <div>
          ...
          <input className="input w-full mt-1 tabular-nums" type="number" step={jePrebytek ? "5" : "1000"} min={jePrebytek ? 0 : data.min} max={jePrebytek ? data.surplusMaxPct : Math.min(data.max, data.freeBalance)} value={amount} onChange={(e) => setAmount(e.target.value)}/>
          {!jePrebytek && (<div className="text-sm text-muted mt-1">
        ...
        <div>
          ...
          <textarea className="input w-full mt-1" rows={3} maxLength={500} value={note} onChange={(e) => setNote(e.target.value)} placeholder="Proč by to kluby měly podpořit?"/>
        </div>
        ...
        <div className="flex gap-2">
          <button className="btn btn-lg btn-secondary flex-1" onClick={onClose}>Zrušit</button>
          <button className="btn btn-lg btn-primary flex-1" disabled={saving || (!!spec?.targeted && !target)} onClick={submit}>
...
export function SponsorPanel({ data, teamId, onChanged }) {
    ...
    const predloz = async (offerId) => {
        if (!teamId)
            return;
        setSaving(offerId);
        const ok = await apiAction(apiFetch(`/api/teams/${teamId}/competition/sponsor-offers/${offerId}/propose`, { method: "POST" }), "Nabídku se nepodařilo předložit");
        setSaving(null);
        if (ok)
            onChanged();
    };
    ...
    return (<div className="card p-5">
      ...
      {data.offers.filter((o) => o.status === "open").length === 0 ? (<p className="text-sm text-muted">
          ...
          <div className="space-y-3">
            {data.offers.filter((o) => o.status === "open").map((o) => (<div key={o.id} className="rounded-lg p-3" style={{ background: "var(--color-paper)" }}>
                <div className="flex items-start justify-between gap-3">
                  ...
                  {smim && (<button className="btn btn-md btn-secondary shrink-0" disabled={saving === o.id} onClick={() => predloz(o.id)}>
                      {saving === o.id ? "Odesílám…" : "Dát hlasovat"}
                    </button>)}
                </div>
                {o.conditions.length > 0 && (<ul className="mt-2 space-y-1">
                    {o.conditions.map((c, i) => (<li key={i} className="text-sm text-muted flex gap-2">
                        <span aria-hidden="true" style={{ color: GOLD }}>◆</span>
          ...
          {!smim && (<p className="text-sm text-muted mt-3">
 ...
 */
export function OdborInbox({ roleKey, roleLabel, roleAkuzativ, messages, maxLength, canPost, teamId, avatars, onPost }) {
    return (<Vlakno title="Vzkazy odboru" right={roleLabel || undefined} subtitle={`Soukromá linka na ${roleAkuzativ || "vedoucího odboru"}. `
            + "Čte to on a prezident soutěže, ostatní kluby ne. Sem patří stížnost nebo nápad, "
            + "který ještě nemá být návrhem na zasedání."} messages={messages} maxLength={maxLength} canPost={canPost} placeholder="Co ti vadí, nebo co bys zlepšil?" teamId={teamId} avatars={avatars} onPost={onPost} key={roleKey}/>);
}
/**
 ...
 */
function Vlakno({ title, subtitle, right, messages, maxLength, canPost, placeholder, teamId, avatars, onPost }) {
    ...
    return (<div className="card p-5" style={{ background: "var(--color-paper)" }}>
      <Ornament right={right}>{title}</Ornament>
      <p className="text-sm text-muted -mt-1">{subtitle}</p>

      <div className="mt-4 space-y-3">
        {messages.length === 0 && (<div className="text-sm text-muted">Zatím tu nikdo nic nenapsal.</div>)}
        {messages.map((m) => {
            const mine = m.teamId === teamId;
            return (<div key={m.id} className="flex gap-3">
              <Portrait avatar={avatars[m.teamId]} name={m.senderName} size={34} ring={mine ? GOLD : "var(--color-muted-light)"}/>
              <div className="min-w-0 flex-1 rounded-lg p-3" style={{
      ...
      {canPost && (<div className="mt-4">
          <textarea className="input w-full" rows={3} maxLength={maxLength} value={text} onChange={(e) => setText(e.target.value)} placeholder={placeholder}/>
...
// ── Zápisy ──────────────────────────────────────────────────────────────────
export function ZapisyPanel({ meetings }) {
    if (meetings.length === 0)
        return <Empty>Grémium se zatím nesešlo.</Empty>;
    return (<div className="space-y-4">
      {meetings.map((m) => (<div key={m.id} className="card p-5">
          ...
          <div className="divide-y" style={{ borderColor: "var(--color-line)" }}>
            {m.items?.map((it, i) => (<div key={i} className="flex items-start justify-between gap-3 py-2">
                <div className="text-sm min-w-0 break-words">
                  ...
                  {typeof it.pro === "number" && typeof it.proti === "number" && (<div className="text-muted tabular-nums">
