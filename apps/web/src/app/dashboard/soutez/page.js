...
import { Empty, GOLD, OpenProposalNote, Ornament, PersonLine, Portrait, Row, StatusPill, czk, plural, signed, } from "./ui";
...
const TAB_KEYS = ["zasedani", "pokladna", "vedeni", "zapisy"];
/** Pořadí určuje pořadí podzáložek. „zadna" je koš pro návrhy mimo gesce. */
const GESCE_ORDER = ["soutez", "hospodarska", "disciplinarni", "rozhodcich", "integrita", "zadna"];
/**
...
const ANSWER_LABEL = { pro: "Pro", proti: "Proti", zdrzel: "Zdržel se" };
/**
...
export default function SoutezPage() {
    ...
    const [gesce, setGesce] = useState("hospodarska");
    const leagueId = state?.league.id ?? null;
    const loadState = useCallback(() => {
        if (!teamId)
            return;
        apiFetch(`/api/teams/${teamId}/competition`)
    ...
    const loadProposals = useCallback(() => {
        if (!leagueId)
            return;
        apiFetch(`/api/competition/${leagueId}/proposals`)
    ...
    const loadElections = useCallback(() => {
        if (!leagueId)
            return;
        apiFetch(`/api/competition/${leagueId}/elections`)
    ...
    const loadDiscipline = useCallback(() => {
        if (!leagueId)
            return;
        apiFetch(`/api/competition/${leagueId}/discipline`)
    ...
    const loadSponsor = useCallback(() => {
        if (!leagueId)
            return;
        apiFetch(`/api/competition/${leagueId}/sponsor`)
    ...
    const postToBoard = useCallback(async (text, scope, targetRole) => {
        if (!teamId)
            return;
        const ok = await apiAction(apiFetch(`/api/teams/${teamId}/competition/board`, {
    ...
    const loadReferees = useCallback(() => {
        if (!leagueId)
            return;
        apiFetch(`/api/competition/${leagueId}/referees`)
    ...
    const avatars = useMemo(() => {
        const out = {};
        for (const t of discipline?.targets ?? [])
            out[t.teamId] = t.managerAvatar;
        return out;
    }, [discipline]);
    /**
    ...
    useEffect(() => {
        if (gesceList.length > 0 && !gesceList.includes(gesce))
            setGesce(gesceList[0]);
    }, [gesceList, gesce]);
    /**
    ...
    useEffect(() => {
        if (vlastniOdborNastaven.current || !state || !teamId)
            return;
        vlastniOdborNastaven.current = true;
        const moje = state.officials.find((o) => o.teamId === teamId && (o.status === "active" || o.status === "suspended"));
        if (!moje)
            return;
        const g = GESCE_ORDER.find((k) => ROLE_OF_GESCE[k] === moje.role);
        if (g && gesceList.includes(g))
            setGesce(g);
    }, [state, teamId, gesceList]);
    const open = useMemo(() => proposals.filter((p) => p.status === "open" && p.gesce === gesce), [proposals, gesce]);
    const closed = useMemo(() => proposals.filter((p) => p.status !== "open" && p.gesce === gesce), [proposals, gesce]);
    ...
    const myOpenByGesce = useMemo(() => {
        ...
        for (const p of openAll) {
            if (p.proposedByTeamId !== teamId || p.kind === "recall")
                continue;
            if (!m[p.gesce])
                m[p.gesce] = { title: p.title };
        }
        return m;
    }, [openAll, teamId]);
    const isChair = useMemo(() => !!state?.officials.find((o) => o.role === "disciplinarni" && o.teamId === teamId && o.status === "active"), [state, teamId]);
    ...
    const jeKomisarRozhodcich = useMemo(() => {
        if (!state || !teamId)
            return false;
        const drzitel = state.officials.find((o) => o.role === "rozhodcich" && (o.status === "active" || o.status === "suspended"));
        if (drzitel)
            return drzitel.teamId === teamId && drzitel.status === "active";
    ...
    const vote = async (proposalId, answer) => {
        if (!teamId)
            return;
        const ok = await apiAction(apiFetch(`/api/teams/${teamId}/competition/proposals/${proposalId}/ballot`, {
            method: "POST", headers: { "Content-Type": "application/json" },
    ...
    const withdraw = async (proposalId) => {
        if (!teamId)
            return;
        const ok = await apiAction(apiFetch(`/api/teams/${teamId}/competition/proposals/${proposalId}`, { method: "DELETE" }), "Stažení návrhu se nezdařilo");
        if (ok)
            refreshAll();
    };
    const defend = async (proposalId, text) => {
        if (!teamId)
            return;
        const ok = await apiAction(apiFetch(`/api/teams/${teamId}/competition/proposals/${proposalId}/defence`, {
            method: "POST", headers: { "Content-Type": "application/json" },
    ...
    <div className="page-container space-y-4 pb-10" style={{ maxWidth: 820 }}>
      ...
      <div className="card overflow-hidden">
        ...
        <div className="p-5">
          ...
          {state.enabled ? (<div className="mt-4 grid grid-cols-3 gap-3">
      ...
      {tab === "zasedani" && (<div className="space-y-4">
          ...
          <Tabs items={gesceList.map((g) => ({
                key: g, label: GESCE_LABEL[g],
                count: proposals.filter((p) => p.status === "open" && p.gesce === g).length || null,
            }))} value={gesce} onChange={setGesce}/>

          <GesceHeader gesce={gesce} state={state} avatars={avatars}/>

          {gesce === "disciplinarni" && (<DisciplinePanel data={discipline} state={state} teamId={teamId} isChair={isChair} myOpen={myOpenByGesce.disciplinarni ?? null} onChanged={refreshAll}/>)}

          {gesce === "integrita" && state.enabled && teamId && (<IntegrityPanel state={state} teamId={teamId} jsemKomisar={state.roles.some((r) => r.role === "integrita" && r.holder?.teamId === teamId)} onChanged={refreshAll}/>)}

          {gesce === "rozhodcich" && (<RefereesPanel data={referees} state={state} teamId={teamId} myOpen={myOpenByGesce.rozhodcich ?? null} jeKomisar={jeKomisarRozhodcich} onChanged={refreshAll}/>)}

          {open.length === 0 ? (<Empty>V tomhle odboru není na programu žádný bod.</Empty>) : (open.map((p) => (<ProposalCard key={p.id} p={p} myTeamId={teamId} avatars={avatars} onVote={vote} onWithdraw={withdraw} onDefend={defend} canVote={state.enabled}/>)))}

          {state.enabled && gesce !== "zadna" && gesce !== "disciplinarni" && gesce !== "rozhodcich" && (myOpenByGesce[gesce] ? (<OpenProposalNote p={myOpenByGesce[gesce]}/>) : (<>
          ...
          {gesce === "hospodarska" && grants && state.enabled && teamId && (<GrantsPanel data={grants} teamId={teamId} myOpen={myOpenByGesce.hospodarska ?? null} onChanged={refreshAll}/>)}

          {gesce === "soutez" && sponsor && (<SponsorPanel data={sponsor} teamId={teamId} onChanged={refreshAll}/>)}

          {board && ROLE_OF_GESCE[gesce] && (<OdborInbox roleKey={ROLE_OF_GESCE[gesce]} roleLabel={state.roles?.find((r) => r.role === ROLE_OF_GESCE[gesce])?.label ?? ""} roleAkuzativ={state.roles?.find((r) => r.role === ROLE_OF_GESCE[gesce])?.labelAkuzativ ?? ""} messages={board.inbox?.[ROLE_OF_GESCE[gesce]] ?? []} maxLength={board.maxLength} canPost={board.canPost} teamId={teamId} avatars={avatars} onPost={(t) => postToBoard(t, "odbor", ROLE_OF_GESCE[gesce])}/>)}

          {closed.length > 0 && (<div className="pt-2">
              <Ornament>Projednané body</Ornament>
              <div className="space-y-4">
                {closed.map((p) => (<ProposalCard key={p.id} p={p} myTeamId={teamId} avatars={avatars} onVote={vote} onWithdraw={withdraw} onDefend={defend} canVote={false}/>))}
              </div>
            </div>)}
      ...
      {tab === "vedeni" && (<VedeniPanel state={state} elections={elections} avatars={avatars} board={board} teamId={teamId} onChanged={refreshAll} onPost={postToBoard}/>)}

      {tab === "zapisy" && <ZapisyPanel meetings={meetings}/>}

      {formOpen && teamId && (<ProposalForm teamId={teamId} state={state} gesce={gesce} onClose={() => setFormOpen(false)} onSaved={() => { setFormOpen(false); refreshAll(); }}/>)}
    </div>);
}
function Stat({ label, value }) {
    return (<div className="rounded-lg p-2.5 text-center" style={{ background: "var(--color-paper)" }}>
...
function GesceHeader({ gesce, state, avatars }) {
    ...
    return (<div className="card p-5">
      ...
      {slot && (<div className="mt-3 flex items-center gap-3">
          <Portrait avatar={slot.holder ? avatars[slot.holder.teamId] : null} name={slot.holder?.managerName} size={44} ring={slot.holder ? GOLD : "var(--color-muted-light)"}/>
          {slot.holder ? (<PersonLine managerName={slot.holder.managerName} teamId={slot.holder.teamId} teamName={slot.holder.teamName} note={slot.holder.status === "suspended"
                    ? <span className="text-danger">pravomoc pozastavena — odbor vede prezident</span>
                    : slot.label.toLowerCase()}/>) : (<div className="text-sm text-muted">
...
function ProposalCard({ p, myTeamId, avatars, onVote, onWithdraw, onDefend, canVote }) {
    ...
    return (<div className="card overflow-hidden">
      {p.status !== "open" && (<div className="h-0.5" style={{ background: p.status === "passed" ? "#3B7A3B" : "#D94032" }}/>)}
      <div className="p-5 space-y-3">
        ...
        {p.evidence && (<div className="text-sm rounded-lg p-3" style={{ background: "var(--color-paper)" }}>
        ...
        {p.defence && (<div className="text-sm rounded-lg p-3" style={{ background: "#FBF6E6" }}>
        ...
        {p.status === "open" ? (<>
            ...
            {jsemDotceny ? (<div className="space-y-2">
                <div className="text-sm">
                  O návrhu, který se týká tvého klubu, nehlasuješ. Můžeš ale napsat obhajobu —
                  uvidí ji všichni před hlasováním.
                </div>
                {!p.defence && (<>
                    <textarea className="input w-full" rows={2} maxLength={150} value={defence} onChange={(e) => setDefence(e.target.value)} placeholder="Co na svou obranu…"/>
                    <button className="btn btn-lg btn-secondary w-full" disabled={!defence.trim()} onClick={() => onDefend(p.id, defence)}>
                      Odeslat obhajobu
                    </button>
                  </>)}
              </div>) : canVote ? (<div className="grid grid-cols-3 gap-2">
                {["pro", "proti", "zdrzel"].map((a) => (<button key={a} onClick={() => onVote(p.id, a)} className={`btn btn-md ${p.myAnswer === a ? "btn-primary" : "btn-secondary"}`}>
                    {ANSWER_LABEL[a]}
                  </button>))}
              </div>) : null}

            {p.myAnswer && (<div className="text-sm text-muted">
                Tvůj hlas: {ANSWER_LABEL[p.myAnswer]} — do zahájení zasedání ho můžeš změnit.
              </div>)}
            {mine && (<button className="btn btn-md btn-ghost w-full" onClick={() => onWithdraw(p.id)}>
                Stáhnout návrh
              </button>)}
          </>) : (<>
            {p.resultNote && <div className="text-sm">{p.resultNote}</div>}
            {p.voters && p.voters.length > 0 && (<div className="text-sm space-y-1">
                {["pro", "proti", "zdrzel"].map((a) => {
                    const list = p.voters.filter((v) => v.answer === a);
                    if (list.length === 0)
                        return null;
                    return (<div key={a} className="break-words">
                      <span className="text-muted">{ANSWER_LABEL[a]}: </span>
                      {list.map((v, i) => (<span key={v.teamId}>
                          {i > 0 && ", "}
...
function ProposalForm({ teamId, state, gesce, onClose, onSaved }) {
    ...
    const kinds = useMemo(() => state.proposalKinds.filter((k) => k.gesce === gesce), [state.proposalKinds, gesce]);
    const [kind, setKind] = useState(kinds[0]?.kind ?? "");
    const spec = kinds.find((k) => k.kind === kind);
    const [value, setValue] = useState(String(spec?.current ?? 0));
    ...
    useEffect(() => {
        ...
        const t = setTimeout(() => {
            apiFetch(`/api/teams/${teamId}/competition/impact?kind=${kind}&value=${v}`)
                .then(setImpact)
    ...
    const mimoRozsah = (() => {
        ...
        if (spec.min !== null && v < spec.min)
            return `Nejmíň ${fmtMez(spec, spec.min)}.`;
        if (spec.max !== null && v > spec.max)
            return `Nejvíc ${fmtMez(spec, spec.max)}.`;
        return null;
    })();
    const submit = async () => {
        if (mimoRozsah)
            return;
        setSaving(true);
        const ok = await apiAction(apiFetch(`/api/teams/${teamId}/competition/proposals`, {
            method: "POST", headers: { "Content-Type": "application/json" },
    ...
    return (<Modal isOpen onClose={onClose} title="Nový návrh" zavritKlikemVedle={false}>
      <div className="p-5 space-y-4">
        ...
        {spec && (<div>
            ...
            {spec.unit === "switch" ? (<>
                ...
                ].map((o) => (<button key={o.v} type="button" className={`btn btn-lg ${value === o.v ? "btn-primary" : "btn-secondary"}`} onClick={() => setValue(o.v)}>
                      {o.label}
                    </button>))}
                </div>
              </>) : (<>
                ...
                <input className="input w-full mt-1 tabular-nums" type="number" step={spec.unit === "ratio" ? "0.01" : "1"} min={spec.min ?? undefined} max={spec.max ?? undefined} value={value} onChange={(e) => setValue(e.target.value)}/>
        ...
        {impact && zmeneno && (impact.delta.matchBonus !== 0 || impact.delta.placeReward !== 0 || impact.delta.entryFee !== 0) && (<div className="rounded-lg p-3 text-sm space-y-1" style={{ background: "var(--color-paper)" }}>
        ...
        <div>
          ...
          <textarea className="input w-full mt-1" rows={3} maxLength={500} value={body} onChange={(e) => setBody(e.target.value)} placeholder="Proč by to kluby měly podpořit?"/>
        </div>
