...
export function DisciplinePanel({ data, state, teamId, isChair, myOpen, onChanged }) {
    ...
    useEffect(() => {
        ...
        let zahozeno = false;
        apiFetch(`/api/teams/${teamId}/competition/pitches`)
            .then((d) => { if (!zahozeno)
            setPitches(d); })
            .catch((e) => {
            if (zahozeno)
    ...
    if (!data)
        return <Empty>Načítám disciplinární agendu…</Empty>;
    const mine = data.sanctions.filter((s) => s.teamId === teamId);
    return (<div className="space-y-4">
      <div className="card p-5">
        <Ornament>Co lze potrestat</Ornament>
        <div className="space-y-2">
          {data.offences.map((o) => (<div key={o.kind} className="text-sm">
      ...
      {state.enabled && teamId && (myOpen ? <OpenProposalNote p={myOpen}/> : (<button className="btn btn-lg btn-primary w-full" onClick={() => setFormOpen(true)}>
            Podat návrh na pokutu
          </button>))}

      {mine.length > 0 && (<div className="card p-5">
          <Ornament>Tresty tvého klubu</Ornament>
          <div className="space-y-3">
            {mine.map((s) => (<SanctionRow key={s.id} s={s} teamId={teamId} onChanged={onChanged} canAppeal/>))}
          </div>
        </div>)}

      {pitches && <PitchOverview pitches={pitches}/>}

      <Rozbaleni title="Listina trestů" right={`${data.sanctions.length} celkem`} note={data.sanctions.length === 0
            ? "Zatím čistý štít, nikdo nedostal pokutu."
            : "Všechny pokuty sezóny i s odůvodněním."}>
        {data.sanctions.length > 0 && (<div className="space-y-3">
            {data.sanctions.map((s) => (<SanctionRow key={s.id} s={s} teamId={teamId} onChanged={onChanged}/>))}
          </div>)}
      </Rozbaleni>

      {formOpen && teamId && (<FineForm data={data} teamId={teamId} isChair={isChair} onClose={() => setFormOpen(false)} onSaved={() => { setFormOpen(false); onChanged(); }}/>)}
    </div>);
}
function SanctionRow({ s, teamId, onChanged, canAppeal }) {
    ...
    const appeal = async () => {
        if (!teamId)
            return;
        setBusy(true);
        const ok = await apiAction(apiFetch(`/api/teams/${teamId}/competition/sanctions/${s.id}/appeal`, { method: "POST" }), "Odvolání se nepodařilo podat");
        setBusy(false);
        if (ok)
            onChanged();
    };
    return (<div className="rounded-lg p-3" style={{ background: "var(--color-paper)" }}>
      ...
      {canAppeal && s.status === "issued" && (<button className="btn btn-md btn-secondary w-full mt-2" disabled={busy} onClick={appeal}>
          Odvolat se
        </button>)}
    </div>);
}
function FineForm({ data, teamId, isChair, onClose, onSaved }) {
    ...
    const targets = useMemo(() => data.targets.filter((t) => t.teamId !== teamId), [data.targets, teamId]);
    ...
    const evidenced = useMemo(() => new Set((chosen?.evidence ?? []).map((e) => e.kind)), [chosen]);
    ...
    const lzeVybrat = (kind) => evidenced.has(kind) || data.offences.find((o) => o.kind === kind)?.freeText === true;
    ...
    const submit = async () => {
        ...
        const ok = await apiAction(apiFetch(url, {
    ...
    return (<Modal isOpen onClose={onClose} title="Návrh na pokutu" zavritKlikemVedle={false}>
      <div className="p-5 space-y-4">
        ...
        <div>
          ...
          <select className="select w-full mt-1" value={target} onChange={(e) => setTarget(e.target.value)}>
            {targets.map((t) => (<option key={t.teamId} value={t.teamId}>
        ...
        {chosen && (<div className="flex items-center gap-3 rounded-lg p-3" style={{ background: "var(--color-paper)" }}>
        ...
        <div>
          ...
          <select className="select w-full mt-1" value={offence} onChange={(e) => setOffence(e.target.value)}>
            {options.map((o) => (<option key={o.kind} value={o.kind} disabled={!lzeVybrat(o.kind)}>
                {o.label}{lzeVybrat(o.kind) ? "" : `, u tohohle klubu nedoložíš (${o.evidenceLabel})`}
              </option>))}
          </select>
          {nicNelzeDolozit && (<div className="text-sm text-muted mt-1">
        ...
        {spec?.freeText && (<div>
            ...
            <textarea className="input w-full mt-1" rows={3} maxLength={200} value={note} onChange={(e) => setNote(e.target.value)} placeholder="Popiš, co klub provedl. Bez toho návrh neprojde."/>
          </div>)}
        ...
        <div>
          ...
          <input className="input w-full mt-1 tabular-nums" type="number" step="500" min={data.limits.min} max={data.limits.max} value={amount} onChange={(e) => setAmount(e.target.value)}/>
        </div>

        {isChair && (<label className="flex items-start gap-2 text-sm">
 ...
 */
function PitchOverview({ pitches }) {
    ...
    return (<Rozbaleni title="Stav hřišť" right={`${teams.length} ${plural(teams.length, "klub", "kluby", "klubů")}`} note={note}>
      ...
      {bezHriste > 0 && (<p className="text-sm text-muted mt-3 leading-snug">
...
function pitchColor(pitch, limit) {
    if (pitch === null)
        return "var(--color-muted)";
    if (limit > 0 && pitch < limit)
        return "#A32B1F";
    if (limit > 0 && pitch < limit + 10)
        return "#A88A2A";
    if (pitch < 40)
        return "#A88A2A";
    return "#1E4A1E";
}
function PitchLine({ t, limit }) {
    ...
    return (<div className="flex items-center gap-3 py-2.5">
      <div className="min-w-0 flex-1">
        ...
        <div className="h-1.5 rounded-full mt-1.5 overflow-hidden" style={{ background: "var(--color-line)" }}>
          <div className="h-full rounded-full" style={{ width: `${Math.max(0, Math.min(100, t.pitch ?? 0))}%`, background: barva }}/>
        </div>
      </div>
