...
const Stadium3D = dynamic(() => import("@/components/stadium/stadium-3d/Stadium3D").then((m) => m.Stadium3D), {
    ssr: false,
    loading: () => (<div className="h-full flex items-center justify-center text-muted text-sm">
        <Spinner />
      </div>),
});
const Stadium3DViewer = dynamic(() => import("@/components/stadium/stadium-3d/Stadium3DViewer").then((m) => m.Stadium3DViewer), { ssr: false });
...
function pitchColor(condition) {
    if (condition >= 80)
        return "text-pitch-500";
    if (condition >= 60)
        return "text-pitch-600";
    if (condition >= 40)
        return "text-gold-600";
    return "text-card-red";
}
function pitchBarColor(condition) {
    if (condition >= 80)
        return "bg-pitch-400";
    if (condition >= 60)
        return "bg-pitch-500";
...
function Sekce({ titul, doplnek, children }) {
    return (<section className="card p-4 sm:p-5 space-y-4">
...
function Podnadpis({ children, doplnek }) {
    return (<div className="flex flex-wrap items-baseline justify-between gap-x-3 gap-y-0.5">
...
function Radek({ ikona, nazev, popis, akce }) {
    return (<div className="flex flex-col gap-2 py-3 border-b border-gray-100 last:border-b-0 sm:flex-row sm:items-center sm:justify-between sm:gap-4">
...
function Dlazdice({ vybrano, disabled, onClick, ikona, nazev, popis, meta }) {
    return (<button onClick={onClick} disabled={disabled} className={`flex flex-col text-left p-3 rounded-soft border-2 transition-colors min-h-[76px] ${vybrano
...
function Prepinac({ hodnoty, aktivni, onChange, disabled }) {
    return (<div className="grid gap-1.5 p-1 bg-gray-100 rounded-soft" style={{ gridTemplateColumns: `repeat(${hodnoty.length}, minmax(0, 1fr))` }}>
      {hodnoty.map((h) => (<button key={h.id} onClick={() => onChange(h.id)} disabled={disabled} className={`py-2.5 px-3 rounded-soft text-sm font-heading font-bold transition-colors ${aktivni === h.id ? "bg-pitch-500 text-white shadow-sm" : "text-muted hover:text-ink"}`}>
          {h.label}
        </button>))}
    </div>);
}
/** Jeden údaj v přehledu areálu. */
function Udaj({ hodnota, popisek, barva }) {
...
export default function StadiumPage() {
    ...
    const [ultrasDraft, setUltrasDraft] = useState(null);
    ...
    const refresh = async () => {
        ...
        const [s, t, sp] = await Promise.all([
            apiFetch(`/api/teams/${teamId}/stadium`),
            apiFetch(`/api/teams/${teamId}`),
            apiFetch(`/api/teams/${teamId}/sponsors`).catch((e) => { console.warn("sponsors fetch:", e); return null; }),
        ]);
        setStadium(s);
        setTeam(t);
    ...
    const handleSurroundChange = async (s) => {
        const current = stadium?.customization.surroundSurface ?? "grass";
        if (!team || current === s.id || acting)
            return;
    ...
    const handleVisualUpgrade = async (kind, label, cost) => {
        if (!teamId || acting)
            return;
        const ok = await confirm({
            title: `Pořídit: ${label}?`,
    ...
    const handleUpgrade = async (facility, label, cost, effect) => {
        if (!teamId || acting)
            return;
        const ok = await confirm({
    ...
    if (!stadium || !team)
        return <div className="page-container">Stadion nenalezen.</div>;
    return (<div className="page-container space-y-5">
      {confirmDialog}

      <Stadium3DViewer open={viewerOpen} onClose={() => setViewerOpen(false)} pitchCondition={stadium.pitchCondition} pitchType={stadium.pitchType} facilities={stadium.facilities} pitchHeating={stadium.pitchHeating ?? 0} pitchIrrigation={stadium.pitchCare?.irrigationLevel ?? 0} mowerLevel={stadium.mowerLevel ?? 2} snowClearingOrdered={stadium.pitchCare?.snowClearingOrdered ?? false} pitchMoisture={stadium.pitchMoisture ?? 50} initialWeather={matchWeather ?? "cloudy"} initialMode={stadium.matchDay ? "match_day" : "training_day"} teamColor={team.primary_color} secondaryColor={team.secondary_color} badgePattern={team.badge_pattern} badgeInitials={team.badge_initials || teamInitials(team.name)} badgeSymbol={team.badge_symbol} badgePrimary={team.badge_primary_color} badgeSecondary={team.badge_secondary_color} stadiumName={stadium.stadiumName} sponsors={sponsorNames} customization={stadium.customization}/>

      {/* ═══ Stadium visualization + stats ═══ */}
      <div className="card p-4 sm:p-5 space-y-4">
        <header className="flex flex-wrap items-baseline justify-between gap-x-3 gap-y-1">
          <h2 className="font-heading font-bold text-lg">{stadium.stadiumName ?? "Areál"}</h2>
          {stadium.currentWeather && (<span className="text-sm text-muted">
        ...
        <Prepinac hodnoty={[{ id: "2d", label: "2D plán" }, { id: "3d", label: "3D areál" }]} aktivni={viewMode} onChange={switchView}/>

        {viewMode === "3d" ? (<div className="space-y-2">
            <div className="h-[280px] sm:h-[500px] rounded-xl overflow-hidden bg-gradient-to-b from-sky-100 to-sky-50 relative" style={{ touchAction: "pan-y" }}>
              {!viewerOpen && (<Stadium3D pitchCondition={stadium.pitchCondition} pitchType={stadium.pitchType} facilities={stadium.facilities} pitchHeating={stadium.pitchHeating ?? 0} pitchIrrigation={stadium.pitchCare?.irrigationLevel ?? 0} mowerLevel={stadium.mowerLevel ?? 2} snowClearingOrdered={stadium.pitchCare?.snowClearingOrdered ?? false} pitchMoisture={stadium.pitchMoisture ?? 50} initialWeather={matchWeather ?? "cloudy"} initialMode={stadium.matchDay ? "match_day" : "training_day"} teamColor={team.primary_color} secondaryColor={team.secondary_color} badgePattern={team.badge_pattern} badgeInitials={team.badge_initials || teamInitials(team.name)} badgeSymbol={team.badge_symbol} badgePrimary={team.badge_primary_color} badgeSecondary={team.badge_secondary_color} stadiumName={stadium.stadiumName} sponsors={sponsorNames} customization={stadium.customization}/>)}
            ...
            <button onClick={() => setViewerOpen(true)} className="w-full py-2 bg-pitch-500 hover:bg-pitch-600 text-white rounded-soft text-sm font-heading font-bold transition-colors shadow-sm">
              🔍 Prohlédnout v plné velikosti
            </button>
            {/* Proč areál vypadá jinak než včera — bez tohohle by přepnutí režimu
                vypadalo jako rozbitá scéna. */}
            {stadium.matchDay && (<div className="text-center text-sm text-ink font-heading">
          ...
          </div>) : (<StadiumView pitchCondition={stadium.pitchCondition} pitchType={stadium.pitchType} facilities={stadium.facilities} teamColor={team.primary_color} mowingPattern={stadium.customization.mowingPattern ?? "stripes"} surroundSurface={stadium.customization.surroundSurface ?? "grass"}/>)}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-y-4 gap-x-3 pt-4 border-t border-gray-100">
          ...
          <Udaj hodnota={`${stadium.pitchMoisture ?? 50} %`} popisek={`Vlhkost · ${moistureLabel(stadium.pitchMoisture).label.toLowerCase()}`} barva={moistureLabel(stadium.pitchMoisture).color}/>
          <Udaj hodnota={stadium.pitchType === "natural" ? "Přírodní" : stadium.pitchType === "hybrid" ? "Hybridní" : "Umělý"} popisek="Povrch"/>
        </div>
        {moistureHint(stadium.pitchMoisture) && (<p className="text-sm text-muted -mt-1">{moistureHint(stadium.pitchMoisture)}</p>)}

        {/* ─── Vzhled stadionu (jen v 3D) ─── */}
        {viewMode === "3d" && (<div className="mt-4 pt-4 border-t border-gray-100 space-y-4">
            ...
            <div className="flex flex-wrap gap-1.5">
              {CUSTOM_FIELDS.filter((f) => (!f.requiresKotel || (stadium.facilities.ultras_stand ?? 0) > 0) && (!f.requiresFlag || (stadium.customization.flagSize ?? 0) > 0)).map(({ field, label, defaultColor, teamDefault }) => {
                ...
                return (<button key={field} onClick={() => setOpenPicker(isActive ? null : field)} className={`flex items-center gap-2 px-3 py-2 rounded-soft border-2 transition-colors text-sm font-heading font-bold ${isActive ? "border-pitch-500 bg-pitch-50" : "border-gray-200 hover:border-gray-400 bg-white"}`}>
            ...
            {openPicker && (() => {
                ...
                if (!cf)
                    return null;
                const current = stadium.customization[openPicker];
                return (<div className="p-3 bg-pitch-50 border-2 border-pitch-500 rounded-soft flex flex-wrap gap-3">
                  <button onClick={() => { handleCustomize(openPicker, null); setOpenPicker(null); }} className={`w-10 h-10 rounded-control border-2 flex items-center justify-center text-sm ${current === null ? "border-pitch-500" : "border-gray-300"}`} style={{ backgroundColor: cf.teamDefault ? (team?.primary_color ?? cf.defaultColor) : cf.defaultColor }} title="Výchozí">
                    ✕
                  </button>
                  {COLOR_PALETTE.map((c) => (<button key={c} onClick={() => { handleCustomize(openPicker, c); setOpenPicker(null); }} className={`w-10 h-10 rounded-control border-2 ${current === c ? "border-pitch-500" : "border-gray-200 hover:border-gray-400"}`} style={{ backgroundColor: c }}/>))}
                </div>);
            })()}

            {/* Nápis v kotli — jen když je postavený sektor kotle */}
            {(stadium.facilities.ultras_stand ?? 0) > 0 && (<div className="pt-2 border-t border-gray-50">
                <Podnadpis>Nápis v kotli</Podnadpis>
                <div className="flex items-center gap-2">
                  <input type="text" maxLength={22} value={ultrasDraft ?? stadium.customization.ultrasText ?? ""} onChange={(e) => setUltrasDraft(e.target.value)} placeholder="např. PRALES BOHDALEC" className="flex-1 min-w-0 border border-gray-200 rounded-soft px-3 py-1.5 text-sm font-heading uppercase"/>
            ...
            <div className="pt-2 border-t border-gray-50">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                {MOWING_PATTERNS.map((p) => (<Dlazdice key={p.id} vybrano={(stadium.customization.mowingPattern ?? "stripes") === p.id} onClick={() => handleCustomize("mowingPattern", p.id)} ikona={p.icon} nazev={p.label} popis={p.desc}/>))}
              </div>
            </div>
            ...
            <div className="pt-2 border-t border-gray-50 space-y-3">
              <div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                  {NET_PATTERNS.map((p) => (<Dlazdice key={p.id} vybrano={(stadium.customization.netPattern ?? "white") === p.id} onClick={() => handleCustomize("netPattern", p.id)} ikona={p.icon} nazev={p.label} popis={p.desc}/>))}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {NET_STYLES.map((st) => (<Dlazdice key={st.id} vybrano={(stadium.customization.netStyle ?? "loose") === st.id} onClick={() => handleCustomize("netStyle", st.id)} ikona={st.icon} nazev={st.label} popis={st.desc}/>))}
                </div>
              </div>
              ...
              <div>
                ...
                return (<Dlazdice key={sur.id} vybrano={vybrano} disabled={!!acting || (!vybrano && !dostupne)} onClick={() => handleSurroundChange(sur)} ikona={sur.icon} nazev={sur.label} popis={sur.desc} meta={vybrano ? (<span className="text-sm font-heading font-bold text-pitch-600">Položeno</span>) : zaplaceno ? (<span className="text-sm font-heading font-bold text-pitch-600">Zaplaceno</span>) : (<span className={`text-sm font-heading font-bold tabular-nums ${sur.cost === 0 ? "text-pitch-600" : dostupne ? "text-ink" : "text-card-red"}`}>
            ...
            <div className="pt-2 border-t border-gray-50 space-y-1">
              ...
              {([
                {
                    kind: "scoreboard", ikona: "🏟️", nazev: "Scoreboard",
                ...
                {
                    kind: "flag", ikona: "🚩", nazev: "Vlajka týmu",
                ...
                return (<Radek key={v.kind} ikona={v.ikona} nazev={<>{v.nazev} <span className="text-muted font-normal">L{v.uroven}/3</span></>} popis={u ? `${v.stav} · ${u.label} za ${formatCZK(u.cost)}` : v.stav} akce={u && (<button onClick={() => handleVisualUpgrade(v.kind, u.label, u.cost)} disabled={!dostupne || !!acting} className="btn btn-primary btn-sm">
      ...
      {/* ═══ Pitch maintenance ═══ */}
      {(stadium.pitchActions.length > 0 || stadium.pitchUpgrades.length > 0) && (<Sekce titul="Údržba trávníku" doplnek={stadium.matchDay ? "dnes se hraje doma" : undefined}>
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              <div className="relative w-full bg-gray-200 rounded-full h-2.5">
                <div className="h-2.5 rounded-full transition-all bg-gradient-to-r from-amber-400 via-pitch-400 to-sky-500" style={{ width: `${stadium.pitchMoisture ?? 50}%` }}/>
          ...
          {stadium.pitchActions.length > 0 && (<div className="space-y-2 mb-3">
              {stadium.pitchActions.map((a) => {
                    const canAfford = team.budget >= a.cost;
                    return (<Radek key={a.level} ikona="🌿" nazev={a.label} popis={`${a.desc} · +${a.improvement} % stav · ${formatCZK(a.cost)}`} akce={<button onClick={async () => {
                                const ok = await confirm({
                                    title: a.label,
                                    description: a.desc,
                                    details: [
                                ...
                                await refresh();
                                setActing(null);
                            }} disabled={!canAfford || !!acting} className="btn btn-primary btn-sm">
                      {acting === "pitch-" + a.level ? "…" : "Provést"}
                    </button>}/>);
                })}
            </div>)}

          {stadium.pitchCare && (<div className="pt-3 border-t border-gray-100">
              <Podnadpis>Péče o trávník</Podnadpis>

              {stadium.pitchCare.heatingLevel === 0 && stadium.pitchCare.irrigationLevel === 0 ? (<p className="text-sm text-muted mb-3">
                  ...
                  <div className="text-sm space-y-1.5">
                    {stadium.pitchCare.heatingLevel > 0 && (<div className="flex flex-wrap items-baseline justify-between gap-x-3">
                        ...
                        <span className="font-heading font-bold tabular-nums">
                          ...
                          {stadium.pitchCare.heatingDailyCost != null && (<> · {formatCZK(stadium.pitchCare.heatingDailyCost)} <span className="text-muted font-normal">za den</span></>)}
                        </span>
                      </div>)}
                    {stadium.pitchCare.irrigationLevel > 0 && (<div className="flex flex-wrap items-baseline justify-between gap-x-3">
                        ...
                        <span className="font-heading font-bold tabular-nums">
                          ...
                          {stadium.pitchCare.irrigationDailyCost != null && (<> · {formatCZK(stadium.pitchCare.irrigationDailyCost)} <span className="text-muted font-normal">za den</span></>)}
                        </span>
                      </div>)}
                  </div>
                  ...
                  <Prepinac hodnoty={[
                        ...
                        { id: "off", label: "Nezapínat" },
                    ]} aktivni={stadium.pitchCare.mode} disabled={!!acting} onChange={async (mode) => {
                        setActing("care-mode");
                        try {
                  ...
                  {stadium.pitchCare.mode === "off" && (<p className="text-sm text-amber-700 mb-3">
                      Péče je vypnutá. Ušetříš za provoz, ale trávník půjde dolů rychleji — a soutěž
                      si může neudržované hřiště vytknout.
                    </p>)}

                  {stadium.pitchCare.mode === "manual" && (<Radek nazev="Péče na příští domácí zápas" popis={stadium.pitchCare.careOrdered ? "Objednaná, zapne se sama." : "Neobjednaná."} akce={<button onClick={async () => {
                            ...
                            }} disabled={!!acting} className="btn btn-primary btn-sm">
                        {acting === "care-order" ? "…" : stadium.pitchCare.careOrdered ? "Zrušit" : "Objednat"}
                      </button>}/>)}
                </>)}

              <Radek ikona="❄️" nazev="Úklid sněhu" popis={`Parta s lopatami na jeden zápas. Zabere i bez vyhřívání, ale rozbředlý podklad neřeší. ${formatCZK(stadium.pitchCare.snowClearingCost)}, platí se až u zápasu.`} akce={<button onClick={async () => {
                        setActing("snow");
                        try {
                            await apiFetch(`/api/teams/${teamId}/stadium/pitch-care-order`, {
          ...
          {stadium.pitchUpgrades.length > 0 && (<div className="pt-3 border-t border-gray-100">
              <Podnadpis>Upgrade povrchu</Podnadpis>
              {stadium.pitchUpgrades.map((u) => {
                    const canAfford = team.budget >= u.cost;
                    return (<Radek key={u.pitchType} ikona={u.pitchType === "hybrid" ? "🌾" : "🟩"} nazev={u.label} popis={`${u.desc} · ${formatCZK(u.cost)}`} akce={<button onClick={async () => {
                                const ok = await confirm({
            ...
            return (<div key={key} className="card p-4">
              ...
              <div className="flex items-center gap-2.5 mb-3">
                ...
                <div className="flex gap-1 shrink-0">
                  {[1, 2, 3].map((l) => (<div key={l} className={`w-3 h-3 rounded-full ${l <= level ? "bg-pitch-400" : "bg-gray-200"}`}/>))}
                </div>
              </div>

              {/* Upgrade info + action */}
              {upgrade && !upgrade.locked && (<div className="flex flex-col gap-2 mt-1 sm:flex-row sm:items-center sm:justify-between sm:gap-3">
                  <div className="min-w-0">
                  ...
                  <button onClick={() => handleUpgrade(upgrade.facility, upgrade.label, upgrade.cost, upgrade.effect)} disabled={!canUpgrade || !!acting} className="btn btn-primary btn-sm w-full sm:w-auto">
                    {acting === key ? "…" : "Koupit"}
                  </button>
                </div>)}
              {upgrade?.locked && (<LockDetail detail={upgrade.lockDetail} hint={upgrade.lockHint} fallback={upgrade.lockReason}/>)}
              {!upgrade && level === 3 && (<div className="text-sm text-pitch-600 font-heading font-bold mt-2">Maximální úroveň</div>)}
            </div>);
        })}
      </div>
    </div>);
