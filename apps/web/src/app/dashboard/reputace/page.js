...
const CZK = (n) => `${n.toLocaleString("cs")} Kč`;
function barColor(v) {
    if (v >= 70)
        return "bg-pitch-500";
    if (v >= 55)
...
export default function ReputacePage() {
    ...
    useEffect(() => {
        if (!teamId)
            return;
        apiFetch(`/api/teams/${teamId}/reputation`)
    ...
    const nextFacility = next ? data.village.facilities.find((f) => (next.kind === "stadium" && ["pitch", "showers", "parking", "stands"].includes(f.key))) : null;
    return (<div className="page-container space-y-5">
      ...
      <div className="card p-4 sm:p-5">
        ...
        <div className="flex items-end justify-between gap-4 mb-1">
          ...
          <div className="text-right">
            ...
            {data.gainFactor < 1 && (<div className="text-sm text-gold-600 tabular-nums">
      ...
      <div className="card p-4 sm:p-5">
        ...
        {next ? (<div className="rounded-soft p-4 mb-4 bg-pitch-50 border-l-2 border-pitch-500">
            ...
            {next.missingReputation > 0 && nextFacility && (<div className="text-sm text-ink-light mt-3 pt-3 border-t border-pitch-500/15">
          ...
          </div>) : (<div className="rounded-soft p-4 mb-4 bg-pitch-50 border-l-2 border-pitch-500 text-sm font-heading font-bold text-pitch-600">
            Máš odemčené všechno. Reputace teď pracuje jen pro peníze a přestupy.
          </div>)}

        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
          {data.unlocks.all.map((u) => (<div key={`${u.kind}-${u.level}`} className={`rounded-soft border p-3 ${u.unlocked ? "border-gray-100 bg-gray-50" : "border-gray-200"}`}>
      ...
      <div className="card p-4 sm:p-5">
        ...
        {data.atNextUnlock && (<div className="text-sm text-muted mb-3">
        ...
        <div className="space-y-1">
          <EarnRow icon="🏪" label="Podpora místních podnikatelů" note="vyplácí se týdně, zaměstnanci ji můžou zvýšit" now={CZK(data.earns.baseSponsorMonthly) + "/měs"} next={data.atNextUnlock ? CZK(data.atNextUnlock.baseSponsorMonthly) + "/měs" : null}/>
          <EarnRow icon="💼" label="Nabídek sponzorů" now={String(data.earns.sponsorOfferCount)} next={data.atNextUnlock ? String(data.atNextUnlock.sponsorOfferCount) : null}/>
          <EarnRow icon="📈" label="Násobič částky od sponzorů" now={`${data.earns.sponsorOfferMultiplier.toFixed(2).replace(".", ",")}×`} next={data.atNextUnlock ? `${data.atNextUnlock.sponsorOfferMultiplier.toFixed(2).replace(".", ",")}×` : null}/>
          <EarnRow icon="📣" label="Bonus k návštěvnosti" now={`+${data.earns.attendanceBonusPct} %`} next={data.atNextUnlock ? `+${data.atNextUnlock.attendanceBonusPct} %` : null}/>
          <EarnRow icon="🍺" label="Pronájem bufetu" note="jen v režimu externího provozovatele" now={CZK(data.earns.concessionWeeklyExternal) + "/týden"} next={data.atNextUnlock ? CZK(data.atNextUnlock.concessionWeeklyExternal) + "/týden" : null}/>
          <EarnRow icon="🤝" label="Zájem hráčů o přestup k tobě" now={`${data.earns.transferInterestScore > 0 ? "+" : ""}${data.earns.transferInterestScore} bodů`} next={data.atNextUnlock ? `${data.atNextUnlock.transferInterestScore > 0 ? "+" : ""}${data.atNextUnlock.transferInterestScore} bodů` : null}/>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        <div className="card p-4 sm:p-5">
          ...
          <div className="space-y-2.5">
            {SOURCES_UP.map((s) => (<SourceRow key={s.label} label={s.label} detail={s.detail} href={s.href} positive/>))}
          </div>
        </div>
        ...
        <div className="card p-4 sm:p-5">
          ...
          <div className="space-y-2.5">
            {SOURCES_DOWN.map((s) => (<SourceRow key={s.label} label={s.label} detail={s.detail} href={null} positive={false}/>))}
          </div>
      ...
      <div className="card p-4 sm:p-5">
        ...
        {data.history.length === 0 ? (<div className="py-6 text-sm text-muted text-center">
            Zatím žádná změna. Historie se začne plnit, jakmile se reputace pohne.
          </div>) : (<div className="space-y-1">
            {data.history.map((h, i) => (<div key={i} className="flex items-start gap-3 py-2 border-b border-gray-50 last:border-b-0">
                <div className={`shrink-0 w-9 h-9 rounded-full flex items-center justify-center font-heading font-bold text-sm tabular-nums ${h.delta > 0 ? "bg-pitch-50 text-pitch-600"
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {h.delta !== h.rawDelta && (<div className="text-sm text-muted">
 ...
 */
function ReputationScale({ reputation, unlocks, baseline, }) {
    ...
    return (<div>
      <div className="relative h-3 rounded-full bg-gray-200 overflow-visible">
        <div className="absolute inset-y-0 left-0 rounded-full bg-gradient-to-r from-pitch-400 to-pitch-500 transition-all" style={{ width: `${Math.max(2, reputation)}%` }}/>
        ...
        {marks.map((m) => (<div key={m.rep} className={`absolute top-1/2 -translate-y-1/2 -translate-x-1/2 w-3 h-3 rounded-full border-2 ${m.reached ? "bg-pitch-600 border-white" : "bg-white border-gray-300"}`} style={{ left: `${m.rep}%` }} title={m.labels.join(" · ")}/>))}
      </div>

      <div className="relative h-5 mt-1">
        {marks.map((m) => (<span key={m.rep} className={`absolute -translate-x-1/2 text-xs tabular-nums ${m.reached ? "text-pitch-600 font-heading font-bold" : "text-muted"}`} style={{ left: `${m.rep}%` }}>
            {m.rep}
          </span>))}
      </div>
...
function ConditionTile({ label, need, have, missing }) {
    const done = missing === 0;
    return (<div className={`rounded-soft p-2.5 ${done ? "bg-white/60" : "bg-white"}`}>
...
function EarnRow({ icon, label, now, next, note }) {
    return (<div className="flex items-baseline gap-3 py-2 border-b border-gray-50 last:border-b-0">
      ...
      {next !== null && (<span className="shrink-0 text-sm tabular-nums text-pitch-600 text-right whitespace-nowrap w-28">
          → {next}
        </span>)}
    </div>);
}
function SourceRow({ label, detail, href, positive, }) {
    return (<div className={`pl-3 border-l-2 ${positive ? "border-pitch-400" : "border-card-red/60"}`}>
      <div className="text-sm font-heading font-bold text-ink">
        {href ? (<Link href={href} className="hover:text-pitch-600 underline decoration-pitch-500/20">{label}</Link>) : label}
      </div>
