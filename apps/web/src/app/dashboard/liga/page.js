...
// ═══ Helpers ═══
const FORM_COLORS = { W: "bg-pitch-400", D: "bg-gold-500", L: "bg-card-red" };
const FORM_LABELS = { W: "V", D: "R", L: "P" };
...
function golPlural(n) { return n === 1 ? "gól" : n >= 2 && n <= 4 ? "góly" : "gólů"; }
// ═══ Main Page ═══
...
function LigaPage() {
    ...
    const initialTab = searchParams.get("tab") || "tabulka";
    ...
    useEffect(() => {
        if (!teamId)
            return;
        apiFetch(`/api/teams/${teamId}/standings`)
            .then((data) => {
    ...
    useEffect(() => {
        ...
        setRounds([]);
        setStatsData(null);
        apiFetch(`/api/leagues/${selectedLeagueId}/standings`)
            .then((data) => {
            setLeagueName(data.leagueName);
            setSeasonNum(data.season ?? null);
            setStandings(data.standings);
    ...
    useEffect(() => {
        if (!teamId || isOtherLeague)
            return;
        if ((tab === "rozpis" || tab === "vysledky") && !loadedTabs.has("rozpis")) {
            apiFetch(`/api/teams/${teamId}/league-schedule`)
                .then((data) => {
        ...
        if (tab === "statistiky" && !loadedTabs.has("statistiky")) {
            apiFetch(`/api/teams/${teamId}/league-stats`)
                .then((data) => {
                setStatsData(data);
    ...
    useEffect(() => {
        if (!isOtherLeague || !selectedLeagueId)
            return;
        if (tab === "zpravodaj" && !loadedTabs.has("zpravodaj")) {
            apiFetch(`/api/leagues/${selectedLeagueId}/news`)
                .then((data) => {
    ...
    const changeTab = (t) => { setTab(t); router.replace(`/dashboard/liga?tab=${t}`, { scroll: false }); };
    ...
    <div className="page-container space-y-5">
      ...
      {(allLeagues.length > 1 || pastSeasons.length > 0) && (<div className="grid grid-cols-2 gap-3 items-end">
      {allLeagues.length > 1 && (<div className="min-w-0">
          <span className="block text-micro text-muted font-heading font-bold uppercase tracking-wide mb-1">Liga</span>
          <select value={selectedLeagueId ?? "own"} onChange={(e) => handleLeagueChange(e.target.value)} className="select w-full">
            <option value="own">Moje liga</option>
            {allLeagues.map((l) => (<option key={l.id} value={l.id}>{l.name} ({l.team_count} týmů)</option>))}
          </select>
        </div>)}

      {/* Season picker — listování minulých sezón této ligy */}
      {pastSeasons.length > 0 && (<div className="min-w-0">
          <span className="block text-micro text-muted font-heading font-bold uppercase tracking-wide mb-1">Sezóna</span>
          <select value={seasonView === "current" ? "current" : String(seasonView)} onChange={(e) => setSeasonView(e.target.value === "current" ? "current" : Number(e.target.value))} className="select w-full">
            <option value="current">Aktuální</option>
            {pastSeasons.map((h) => (<option key={h.id} value={h.seasonNumber}>Sezóna {h.seasonNumber} (archiv)</option>))}
          </select>
        </div>)}
      </div>)}

      {pastEntry ? (<PastSeasonView entry={pastEntry} myTeamId={teamId}/>) : (<>
      {/* Tabs — cizí liga: tabulka + zpravodaj, vlastní: plné menu */}
      <Tabs value={tab} onChange={changeTab} ariaLabel="Liga" items={(isOtherLeague
                ? ["tabulka", "zpravodaj"]
                : ["tabulka", "rozpis", "vysledky", "statistiky"]).map((key) => ({
                key,
      ...
      {isOtherLeague && tab === "zpravodaj" && (!loadedTabs.has("zpravodaj") ? <div className="flex justify-center py-8"><Spinner /></div> : (<div className="space-y-3">
            {newsArticles.length === 0 && <p className="text-muted text-sm text-center py-8">Zatím žádné zprávy</p>}
            {newsArticles.map((a) => (<div key={a.id} className="card p-4">
...
function PastSeasonView({ entry, myTeamId }) {
    const a = entry.awards;
    const s = entry.seasonStats;
    return (<div className="space-y-5">
      {/* Ocenění */}
      {a && (<div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
      ...
      {/* Konečná tabulka */}
      {entry.finalStandings && entry.finalStandings.length > 0 && (<div className="card overflow-x-auto table-scroll">
          <table className="w-full text-sm">
            ...
            <tbody>
              {entry.finalStandings.map((r, idx) => (<tr key={r.teamId} className={`border-b border-gray-50 ${r.teamId === myTeamId ? "bg-pitch-50/60" : idx % 2 === 1 ? "bg-gray-50/30" : ""}`}>
      ...
      {/* Nejlepší jedenáctka */}
      {a?.bestEleven && a.bestEleven.length > 0 && (<div>
          <SectionLabel>Nejlepší jedenáctka</SectionLabel>
          <div className="flex flex-wrap gap-2">
            {a.bestEleven.map((p) => (<div key={p.playerId} className="bg-gray-50 rounded-soft px-3 py-1.5 text-sm">
...
function PastTeamLink({ teamId, name, highlight }) {
    if (!name)
        return <span>—</span>;
    if (!teamId)
        return <span className="font-heading font-bold">{name}</span>;
...
function PastAward({ icon, label, reason, children }) {
    return (<div className="bg-gray-50 rounded-soft px-3 py-2">
...
function PastStat({ label, value, sub }) {
    return (<div className="bg-gray-50 rounded-soft px-3 py-2">
...
function ArticleBody({ article }) {
    if (article.type === "interview") {
        let meta = {};
        try {
            meta = JSON.parse(article.body);
        }
        catch (e) {
            console.error("parse interview body:", e);
        }
        const text = meta.article ?? article.body;
        return (<div className="text-sm text-muted mt-1 space-y-1">
    ...
    if (article.type === "round_results") {
        ...
        {structured.map((r, i) => {
                ...
                const isDraw = h === a;
                const homeWin = h > a;
                return (<div key={i} className="flex items-center text-sm py-1 border-b border-gray-50 last:border-b-0">
              <span className={`flex-1 text-right truncate pr-2 ${homeWin ? "font-heading font-bold" : ""}`}>{r.home}</span>
              <span className={`font-heading font-bold text-xs tabular-nums px-2 py-0.5 rounded min-w-[40px] text-center ${isDraw ? "bg-gray-100 text-muted" : "bg-gray-50 text-ink"}`}>{r.score}</span>
...
function StandingsTab({ standings, teamId }) {
    const [sortKey, setSortKey] = useState("pos");
    const [sortDir, setSortDir] = useState("asc");
    if (standings.length === 0)
        return <div className="card p-8 text-center text-muted">Zatím žádné výsledky.</div>;
    ...
    const sorted = [...standings].sort((a, b) => {
        ...
        switch (sortKey) {
            case "pos":
                ...
                break;
            case "team":
                cmp = a.team.localeCompare(b.team, "cs");
                break;
            case "played":
    ...
    return (<div className="card overflow-x-auto table-scroll">
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b-2 border-gray-200">
            {cols.map((col) => {
            const hideMobile = ["played", "wins", "draws", "losses"].includes(col.key);
            return (<th key={col.key} onClick={() => handleSort(col.key)} colSpan={col.key === "team" ? 2 : undefined} className={`py-2.5 px-1.5 text-xs font-heading uppercase cursor-pointer select-none hover:text-pitch-500 transition-colors whitespace-nowrap ${col.key === "team" ? "text-left pl-3" : "text-center"} ${sortKey === col.key ? "text-pitch-600" : "text-muted"} ${hideMobile ? "hidden sm:table-cell" : ""}`}>
        ...
        <tbody>
          {sorted.map((row, idx) => {
            ...
            return (<tr key={row.teamId ?? row.pos} className={`border-b border-gray-50 transition-colors ${row.isPlayer ? "bg-pitch-50/60" : idx % 2 === 1 ? "bg-gray-50/30" : "hover:bg-gray-50/50"}`}>
                {/* Position */}
                <td className="py-3 px-1.5 text-center">
                  <span className={`font-heading font-[800] text-base tabular-nums ${row.pos === 1 ? "text-gold-500" : isPromo ? "text-pitch-500" : isRel ? "text-card-red" : "text-muted"}`}>
                    {row.pos}
                  </span>
                </td>
                {/* Badge */}
                <td className="py-3 pl-2 pr-0 w-8" style={{ verticalAlign: "middle" }}>
                  <BadgePreview primary={row.primaryColor || "#2D5F2D"} secondary={row.secondaryColor || "#FFF"} pattern={row.badgePattern || "shield"} initials={ini(row.team)} size={24}/>
                </td>
                {/* Team */}
                <td className="py-3 px-1.5" style={{ verticalAlign: "middle" }}>
                  {row.teamId && !row.isAi ? (<Link href={`/dashboard/team/${row.teamId}`} className={`font-heading font-bold hover:text-pitch-500 transition-colors ${row.isPlayer ? "text-pitch-600" : ""}`}>
                      {row.team}
                    </Link>) : (<span className={`font-heading font-bold ${row.isAi ? "text-muted" : ""}`}>{row.team}</span>)}
                </td>
                ...
                <td className="py-3 px-1.5 hidden sm:table-cell">
                  <div className="flex gap-1 justify-center">
                    {(row.form ?? []).slice(0, 5).map((f, i) => (<div key={i} className={`w-5 h-5 rounded ${FORM_COLORS[f] ?? "bg-gray-200"} flex items-center justify-center text-white text-micro font-bold`}>
                        {FORM_LABELS[f] ?? f}
                      </div>))}
                  </div>
                </td>
...
// ═══ Rozpis / Výsledky ═══
function ScheduleTab({ rounds, loaded, teamId, showAll }) {
    if (!loaded)
    ...
    return (<div className="space-y-4">
      {displayRounds.map((round) => (<div key={round.round} className="card overflow-hidden">
          ...
          <div>
            {round.matches.map((m, i) => {
                ...
                const content = (<div className={rowCls}>
                  {/* Mobil — každý tým na svém řádku, aby se vešel celý název */}
                  <div className="sm:hidden flex items-center gap-3">
                    <div className="flex-1 min-w-0 space-y-1">
                      <div className="flex items-center gap-2 h-7 min-w-0">
                        <BadgePreview primary={m.homeColor} secondary={m.homeSecondary} pattern={m.homeBadge} initials={ini(m.homeName)} size={22}/>
                        <span className={`text-base font-heading truncate ${m.homeTeamId === teamId ? "font-bold text-pitch-600" : ""}`}>{m.homeName}</span>
                      </div>
                      <div className="flex items-center gap-2 h-7 min-w-0">
                        <BadgePreview primary={m.awayColor} secondary={m.awaySecondary} pattern={m.awayBadge} initials={ini(m.awayName)} size={22}/>
                        <span className={`text-base font-heading truncate ${m.awayTeamId === teamId ? "font-bold text-pitch-600" : ""}`}>{m.awayName}</span>
                      </div>
                    </div>
                    {isPlayed ? (<div className="shrink-0 space-y-1 text-right font-heading font-[800] text-lg tabular-nums">
                      ...
                      </div>) : (<span className="shrink-0 text-sm text-muted font-heading">vs</span>)}
                  </div>
                  ...
                  <div className="hidden sm:flex items-center">
                    ...
                    <div className="shrink-0 w-20 text-center">
                      {isPlayed ? (<span className="font-heading font-[800] text-lg tabular-nums">{m.homeScore} : {m.awayScore}</span>) : (<span className="text-xs text-muted font-heading">vs</span>)}
                    </div>
                ...
                if (isPlayed)
                    return <Link key={m.id} href={`/dashboard/match/${m.id}`}>{content}</Link>;
...
function StatsTab({ data, loaded }) {
    const [section, setSection] = useState("hraci");
    if (!loaded)
        return <div className="flex items-center justify-center py-12"><Spinner /></div>;
    if (!data)
        return <div className="card p-8 text-center text-muted">Zatím žádné statistiky.</div>;
    const { topScorers, topAssists, topRated, mostCards, mostAppearances, topPenalties = [], topSetPieces = [], topKeepers = [], topCleanSheets = [], topSaves = [], teamPenalties = [], teamSetPieces = [], teamCards = [], mostFouls = [], mostMinutes = [], mostInjuries = [], topAccuracy = [], topGoalsPer90 = [], teamAttack = [], teamDefense = [], teamAttendance = [], teamPossession = [], teamCleanest = [], teamUnbeaten = [], teamWinStreak = [], teamHomeAway = [], curiosities = null, } = data;
    ...
    return (<div className="space-y-4">
    {visible.length > 1 && (<div className="flex gap-1 bg-surface rounded-xl p-1 overflow-x-auto">
        {visible.map((s) => (<button key={s.key} onClick={() => setSection(s.key)} className={`flex-1 whitespace-nowrap px-3 py-2 rounded-soft text-sm font-heading font-bold transition-colors ${active === s.key ? "bg-white text-pitch-600 shadow-sm" : "text-muted hover:text-ink"}`}>{s.label}</button>))}
      </div>)}

    <div className={`grid grid-cols-1 lg:grid-cols-2 gap-4 ${active === "hraci" ? "" : "hidden"}`}>
      {topScorers.length > 0 && (<StatTable title="⚽ Nejlepší střelci" rows={topScorers} valueKey="goals" label="Góly"/>)}
      {topAssists.length > 0 && (<StatTable title="👟 Nejlepší nahrávači" rows={topAssists} valueKey="assists" label="Asist."/>)}
      {topRated.length > 0 && (<StatTable title="⭐ Nejlepší hodnocení" rows={topRated} valueKey="avgRating" label="Hod." decimal/>)}
      {mostAppearances.length > 0 && (<StatTable title="🏃 Nejvíc zápasů" rows={mostAppearances} valueKey="appearances" label="Zápasy"/>)}
      {mostCards.length > 0 && (<StatTable title="🟨 Nejvíc karet" rows={mostCards} valueKey="cards" label="Karty" renderValue={(p) => (<span className="flex items-center gap-1.5 justify-end whitespace-nowrap">
      ...
      {topPenalties.length > 0 && (<StatTable title="🎯 Exekutoři penalt" rows={topPenalties} valueKey="penaltyGoals" label="Penalty" renderValue={(p) => (<span className="flex items-baseline gap-1 justify-end">
            <span>{p.penaltyGoals}</span>
            <span className="text-muted text-sm font-heading font-bold">/{p.penaltyAttempts}</span>
          </span>)}/>)}
      {topSetPieces.length > 0 && (<StatTable title="🥅 Góly ze standardek" rows={topSetPieces} valueKey="setPieceGoals" label="Standardky"/>)}
      {topGoalsPer90.length > 0 && (<StatTable title="⏱ Góly na 90 minut" rows={topGoalsPer90} valueKey="goalsPer90" label="Na 90" renderValue={(p) => (<span className="flex items-baseline gap-1 justify-end">
      ...
      {topAccuracy.length > 0 && (<StatTable title="🎯 Úspěšnost zakončení" rows={topAccuracy} valueKey="shotAccuracy" label="Úspěšnost" renderValue={(p) => (<span className="flex items-baseline gap-1 justify-end">
            <span>{p.shotAccuracy}%</span>
            <span className="hidden sm:inline text-muted text-xs font-heading">{p.goals} z {p.goals + p.chances}</span>
          </span>)}/>)}
      {mostMinutes.length > 0 && (<StatTable title="🕐 Nejvíc odehraných minut" rows={mostMinutes} valueKey="minutesPlayed" label="Minuty" renderValue={(p) => (<span className="flex items-baseline gap-1 justify-end">
            <span>{p.minutesPlayed}</span>
            <span className="hidden sm:inline text-muted text-xs font-heading">min</span>
          </span>)}/>)}
      {mostFouls.length > 0 && (<StatTable title="🦵 Nejvíc faulů" rows={mostFouls} valueKey="fouls" label="Fauly"/>)}
      {mostInjuries.length > 0 && (<StatTable title="🩹 Nejvíc zranění" rows={mostInjuries} valueKey="injuries" label="Zranění"/>)}
    </div>

    {hasKeepers && active === "brankari" && (<div className="space-y-3">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {topKeepers.length > 0 && (<StatTable title="🧤 Nejmíň inkasovaných" rows={topKeepers} valueKey="concededPerMatch" label="Na zápas" renderValue={(p) => (<span className="flex items-baseline gap-1 justify-end">
          ...
          {topCleanSheets.length > 0 && (<StatTable title="🛡 Čistá konta" rows={topCleanSheets} valueKey="cleanSheets" label="Nuly"/>)}
          {topSaves.length > 0 && (<StatTable title="✋ Nejvíc zákroků" rows={topSaves} valueKey="saves" label="Zákroky" renderValue={(p) => (<span className="flex items-baseline gap-1 justify-end">
    ...
    {hasTeams && active === "tymy" && (<div className="space-y-3">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {teamPenalties.length > 0 && (<TeamTable title="🎯 Nejvíc kopaných penalt" rows={teamPenalties} renderValue={(t) => (<span className="flex items-baseline gap-1 justify-end">
          ...
          {teamSetPieces.length > 0 && (<TeamTable title="🥅 Góly ze standardek" rows={teamSetPieces} renderValue={(t) => <span>{t.setPieceGoals}</span>}/>)}
          {teamCards.length > 0 && (<TeamTable title="🟨 Nejvíc karet" rows={teamCards} renderValue={(t) => (<span className="flex items-center gap-1.5 justify-end whitespace-nowrap">
          ...
          {teamAttack.length > 0 && (<TeamTable title="⚔️ Nejlepší útok" rows={teamAttack} renderValue={(t) => (<span className="flex items-baseline gap-1 justify-end">
          ...
          {teamDefense.length > 0 && (<TeamTable title="🛡 Nejlepší obrana" rows={teamDefense} renderValue={(t) => (<span className="flex items-baseline gap-1 justify-end">
          ...
          {teamAttendance.length > 0 && (<TeamTable title="👥 Průměrná návštěva doma" rows={teamAttendance} renderValue={(t) => (<span className="flex items-baseline gap-1 justify-end">
                <span>{t.avgAttendance}</span>
                <span className="hidden sm:inline text-muted text-xs font-heading">diváků</span>
              </span>)}/>)}
          {teamPossession.length > 0 && (<TeamTable title="🔵 Průměrné držení míče" rows={teamPossession} renderValue={(t) => <span>{t.avgPossession}%</span>}/>)}
          {teamCleanest.length > 0 && (<TeamTable title="😇 Nejčistší tým" rows={teamCleanest} renderValue={(t) => (<span className="flex items-baseline gap-1 justify-end">
                <span>{t.fouls}</span>
                <span className="hidden sm:inline text-muted text-xs font-heading">faulů · {t.yellowCards + t.redCards} karet</span>
              </span>)}/>)}
          {teamUnbeaten.length > 0 && (<TeamTable title="🔥 Nejdelší neporazitelnost" rows={teamUnbeaten} renderValue={(t) => (<span className="flex items-baseline gap-1 justify-end">
                <span>{t.longestUnbeaten}</span>
                <span className="hidden sm:inline text-muted text-xs font-heading">zápasů</span>
              </span>)}/>)}
          {teamWinStreak.length > 0 && (<TeamTable title="🏆 Nejvíc výher v řadě" rows={teamWinStreak} renderValue={(t) => (<span className="flex items-baseline gap-1 justify-end">
                <span>{t.longestWinStreak}</span>
                <span className="hidden sm:inline text-muted text-xs font-heading">výher</span>
              </span>)}/>)}
          {teamHomeAway.length > 0 && (<TeamTable title="🏠 Doma vs. venku" rows={teamHomeAway} renderValue={(t) => (<span className="flex flex-col items-end text-sm leading-tight">
    ...
    {hasCuriosities && cur && active === "zajimavosti" && (<div className="space-y-3">
...
function CuriosityCard({ icon, title, item, unit }) {
    if (!item)
        return null;
    return (<Link href={`/dashboard/match/${item.matchId}`} className="card p-3 hover:bg-gray-50 transition-colors block">
...
function TeamTable({ title, rows, renderValue }) {
    return (<div className="card overflow-hidden">
      ...
      <table className="stat-table w-full text-sm table-fixed">
        <tbody>
          {rows.map((t, i) => (<tr key={t.teamId} className={`border-b border-gray-50 last:border-b-0 ${t.isMyTeam ? "bg-pitch-50/40" : ""}`}>
              ...
              <td className="py-2 px-2 min-w-0">
                <Link href={`/dashboard/team/${t.teamId}`} className="flex items-center gap-2 min-w-0 hover:text-pitch-500 transition-colors">
                  <BadgePreview primary={t.teamColor} secondary={t.teamSecondary} pattern={t.teamBadge} initials={ini(t.teamName)} size={18}/>
...
function StatTable({ title, rows, valueKey, label, decimal, renderValue }) {
    return (<div className="card overflow-hidden">
      ...
      <table className="stat-table w-full text-sm table-fixed">
        <tbody>
          {rows.map((p, i) => (<tr key={i} className={`border-b border-gray-50 last:border-b-0 ${p.isMyTeam ? "bg-pitch-50/40" : ""}`}>
              ...
              <td className="py-1.5 px-2 min-w-0 align-top">
                ...
                <div className="flex items-start gap-2 min-w-0">
                  <span className="shrink-0 w-10 flex justify-center pt-px"><PositionBadge position={p.position}/></span>
                  <div className="min-w-0 flex-1">
                    ...
                    <Link href={`/dashboard/team/${p.teamId}`} className="sm:hidden flex items-center gap-1 min-w-0 leading-snug hover:text-pitch-500 transition-colors">
                      <BadgePreview primary={p.teamColor} secondary={p.teamSecondary} pattern={p.teamBadge} initials={ini(p.teamName)} size={12}/>
                  ...
                  <Link href={`/dashboard/team/${p.teamId}`} className="hidden sm:flex items-center gap-1.5 shrink-0 max-w-[45%] hover:text-pitch-500 transition-colors">
                    <BadgePreview primary={p.teamColor} secondary={p.teamSecondary} pattern={p.teamBadge} initials={ini(p.teamName)} size={16}/>
                    <span className="text-xs text-muted truncate">{p.teamName}</span>
                  </Link>
                </div>
              </td>
              <td className="py-1.5 pr-4 w-24 sm:w-36 text-right align-top font-heading font-[800] text-lg leading-snug tabular-nums whitespace-nowrap">
                {renderValue ? renderValue(p) : decimal ? p[valueKey].toFixed(1) : p[valueKey]}
              </td>
            </tr>))}
        </tbody>
      </table>
