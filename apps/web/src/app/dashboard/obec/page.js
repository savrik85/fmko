...
function investmentTargetLabel(facility, type) {
    if (facility && INVESTMENT_TARGET_LABEL[facility])
        return INVESTMENT_TARGET_LABEL[facility];
    if (INVESTMENT_TARGET_LABEL[type])
        return INVESTMENT_TARGET_LABEL[type];
...
function termRemaining(termEndAt) {
    const now = new Date();
    const end = new Date(termEndAt);
    const diffMs = end.getTime() - now.getTime();
    const days = Math.round(diffMs / (1000 * 60 * 60 * 24));
    if (days <= 0)
        return "Funkční období skončilo";
    if (days < 90)
        return `Ve funkci ještě ${days} dní`;
...
export default function ObecPage() {
    ...
    const [favorMatrix, setFavorMatrix] = useState({});
    const [feed, setFeed] = useState([]);
    const [brigades, setBrigades] = useState([]);
    const [petitions, setPetitions] = useState([]);
    const [respondingPetitionId, setRespondingPetitionId] = useState(null);
    const [investments, setInvestments] = useState([]);
    const [respondingInvId, setRespondingInvId] = useState(null);
    const [pubEncounters, setPubEncounters] = useState([]);
    const [respondingPubId, setRespondingPubId] = useState(null);
    const [localPride, setLocalPride] = useState(null);
    const [toast, setToast] = useState(null);
    const [upcoming, setUpcoming] = useState(null);
    const [invitingOfficialId, setInvitingOfficialId] = useState(null);
    const [villageId, setVillageId] = useState(null);
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState(null);
    const [takeBrigade, setTakeBrigade] = useState(null);
    const refresh = async (vid) => {
        if (!teamId)
            return;
        const [v, o, f, ts, fd, bg, up, pe, inv, pub, lp] = await Promise.all([
            apiFetch(`/api/villages/${vid}`),
            apiFetch(`/api/villages/${vid}/officials`),
            apiFetch(`/api/villages/${vid}/favor?teamId=${teamId}`),
            apiFetch(`/api/villages/${vid}/teams`),
            apiFetch(`/api/villages/${vid}/feed?limit=20`),
            apiFetch(`/api/villages/${vid}/brigades`),
            apiFetch(`/api/villages/upcoming-match?teamId=${teamId}`),
            apiFetch(`/api/villages/petitions?teamId=${teamId}`),
            apiFetch(`/api/villages/investments?teamId=${teamId}`),
            apiFetch(`/api/villages/pub-encounters?teamId=${teamId}`),
            apiFetch(`/api/villages/local-pride?teamId=${teamId}`),
        ]);
        setVillage(v);
        setOfficials(o);
        setFavor(f);
        ...
        setBrigades(bg);
        setTeams(ts.teams);
        setFavorMatrix(ts.favorMatrix);
        setUpcoming(up);
        setPetitions(pe);
        setInvestments(inv);
        setPubEncounters(pub);
        setLocalPride(lp);
    };
    const respondPub = async (id, action) => {
        if (respondingPubId)
            return;
        setRespondingPubId(id);
        try {
            const res = await apiFetch(`/api/villages/pub-encounters/${id}/respond`, {
                method: "POST",
            ...
            let msg = "";
            if (action === "ignore")
                msg = "Ignoroval(a) jsi NPC.";
            else if (res.outcome === "scandal")
                msg = `Skandál! ${res.officialName} odešel znechucen.`;
            else
                msg = `Pivo s ${res.officialName}, přízeň +${res.favorDelta}, cena ${res.beerCost} Kč.`;
            setToast(msg);
    ...
    const respondInvestment = async (id, action) => {
        if (respondingInvId)
            return;
        setRespondingInvId(id);
        try {
            const res = await apiFetch(`/api/villages/investments/${id}/respond`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ action }),
            });
    ...
    const respondPetition = async (id, action) => {
        if (respondingPetitionId)
            return;
    ...
    return (<div className="space-y-6 p-4 max-w-5xl mx-auto">
            ...
            return (<Card key={o.id}>
                <CardBody>
                  <div className="flex gap-4">
                    ...
                    return (<div key={t.id} className="flex items-center gap-2 text-xs">
                              <span className="inline-block w-2 h-2 rounded-full border border-gray-200 shrink-0" style={{ backgroundColor: t.primary_color }}/>
          ...
          <CardBody>
            <div className="space-y-3">
              {pubEncounters.map((p) => {
                let face = {};
                try {
                    face = JSON.parse(p.face_config);
                }
                catch {
                    face = {};
                }
                return (<div key={p.id} className="flex items-center gap-3 border border-amber-300 bg-amber-50/40 rounded-soft p-3">
                    ...
                    <div className="flex flex-col gap-1 shrink-0">
                      <Button size="sm" disabled={respondingPubId === p.id} onClick={() => respondPub(p.id, "invite_beer")}>
                        Pozvat na pivo
                      </Button>
                      <Button variant="secondary" size="sm" disabled={respondingPubId === p.id} onClick={() => respondPub(p.id, "ignore")}>
                        Ignorovat
                      </Button>
                    </div>
                  </div>);
              ...
              {upcoming.officials.map((o) => {
                ...
                if (slotTakenByMe) {
                    ...
                    if (myInv.attendance_effects) {
                        try {
                            reason = JSON.parse(myInv.attendance_effects).rejectReason ?? null;
                        }
                        catch {
                            reason = null;
                        }
                    }
                    statusEl = (<div className="text-right max-w-[180px]">
                      <span className="text-xs text-card-red">Odmítl pozvání</span>
                      {reason && (<div className="text-micro text-gray-500 italic mt-0.5 leading-tight">„{reason}"</div>)}
                    </div>);
                }
                return (<div key={o.id} className="border border-gray-200 rounded-soft p-3 flex items-center justify-between gap-3">
                    <div className="min-w-0">
                      <div className="text-xs uppercase tracking-wider text-gray-500">{ROLE_LABEL[o.role]}</div>
                      <div className="text-sm font-semibold truncate">{o.first_name} {o.last_name}</div>
                      <div className="text-xs text-gray-500">přízeň {o.favor}/100 · {PERSONALITY_LABEL[o.personality]}</div>
                    </div>
                    <div className="text-right shrink-0 space-y-1">
                      {statusEl ?? (<>
                          <Button size="sm" disabled={isInviting || !!myInv} onClick={() => handleInvite(o.id)}>
      ...
      {investments.length > 0 && (<Card>
          ...
          <CardBody>
            <div className="space-y-3">
              {investments.map((i) => (<div key={i.id} className="border border-pitch-300 bg-pitch-50/30 rounded-soft p-3">
                  ...
                  <div className="flex justify-end gap-2 mt-2">
                    <Button variant="secondary" size="sm" disabled={respondingInvId === i.id} onClick={() => respondInvestment(i.id, "decline")}>
                      Odmítnout
                    </Button>
                    <Button size="sm" disabled={respondingInvId === i.id} onClick={() => respondInvestment(i.id, "accept")}>
                      Přijmout
                    </Button>
                  </div>
                </div>))}
      ...
      {petitions.length > 0 && (<Card>
          ...
          <CardBody>
            <div className="space-y-3">
              {petitions.map((p) => (<div key={p.id} className="border border-amber-200 bg-amber-50/40 rounded-soft p-3">
                  <div className="flex justify-between items-start gap-3">
                    <div className="min-w-0">
                  ...
                  <div className="flex justify-end gap-2 mt-2">
                    <Button variant="secondary" size="sm" disabled={respondingPetitionId === p.id} onClick={() => respondPetition(p.id, "ignore")}>
                      Ignorovat
                    </Button>
                    <Button size="sm" disabled={respondingPetitionId === p.id} onClick={() => respondPetition(p.id, "accept")}>
                      Vyhovět
                    </Button>
                  </div>
                </div>))}
      ...
      <Card>
        ...
        <CardBody>
          {brigades.length === 0 ? (<div className="text-sm text-gray-500 italic">Žádné aktuální brigády. Nové se vyhlásí v pondělí.</div>) : (<div className="space-y-3">
              {brigades.map((b) => {
                const isOpen = b.status === "open";
                const isMine = b.taken_team_name && teams.find((t) => t.id === teamId)?.name === b.taken_team_name;
                return (<div key={b.id} className={`border rounded-soft p-3 ${isOpen ? "border-pitch-300 bg-pitch-50/30" : "border-gray-200 bg-gray-50/40 opacity-80"}`}>
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex-1 min-w-0">
                        <div className="font-semibold text-sm">{b.title}</div>
                        <div className="text-xs text-gray-600 mt-0.5">{b.description}</div>
                        {b.offering_official_name && (<div className="text-xs text-gray-500 mt-1">
                      ...
                      <div className="text-right text-xs space-y-0.5 shrink-0">
                        ...
                        {b.morale_change !== 0 && (<div className={b.morale_change > 0 ? "text-pitch-600" : "text-card-red"}>
                    ...
                    <div className="flex items-center justify-between mt-3">
                      <div className="text-xs text-gray-500">
                        ...
                        {!isOpen && b.taken_team_name && (<span className="ml-2 font-medium">
                            · Vzal {isMine ? "náš tým" : b.taken_team_name}
                            {b.taken_at && ` (${new Date(b.taken_at).toLocaleDateString("cs")})`}
                          </span>)}
                      </div>
                      {isOpen && (<Button onClick={() => setTakeBrigade(b)} size="sm">
                          Vzít brigádu
                        </Button>)}
                    </div>
                  </div>);
      ...
      <Card>
        ...
        <CardBody>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="text-left text-xs uppercase tracking-wider text-gray-500">
                <tr>
                  ...
                  {officials.map((o) => (<th key={o.id} className="py-2 px-2 text-right whitespace-nowrap">
            ...
            .map((t) => (<tr key={t.id} className={t.id === teamId ? "font-semibold bg-pitch-50/30" : ""}>
                      <td className="py-2 pr-3 sticky left-0 bg-inherit">
                        <span className="inline-flex items-center gap-2">
                          <span className="inline-block w-3 h-3 rounded-full border border-gray-200 shrink-0" style={{ backgroundColor: t.primary_color }}/>
                ...
                return (<td key={o.id} className="py-2 px-2 text-right tabular-nums">
                            <span className={f >= 70 ? "text-pitch-700"
                        : f >= 40 ? "text-gray-700"
                            : "text-card-red"}>
                              {f}
                            </span>
      ...
      <Card>
        ...
        <CardBody>
          {feed.length === 0 ? (<div className="text-sm text-gray-500 italic">Zatím žádné události. Brigády a další akce se objeví po zavedení Sprintu B.</div>) : (<ul className="divide-y divide-gray-100">
              {feed.map((e) => (<li key={e.id} className="py-2 text-sm">
                  <div className="flex justify-between items-baseline gap-2">
                    <span>{e.description}</span>
                    <span className="text-xs text-gray-500 shrink-0">{new Date(e.created_at).toLocaleDateString("cs")}</span>
                  </div>
                  {(e.team_name || e.official_name) && (<div className="text-xs text-gray-500 mt-0.5">
      ...
      {localPride && (<Card>
          ...
          <CardBody>
            {localPride.totalLocalCount === 0 ? (<div className="text-sm text-gray-500 italic">
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {localPride.locals.map((p) => (<tr key={p.id} className="border-t border-gray-100">
      ...
      {toast && (<div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-[var(--z-sheet)] bg-gray-900 text-white px-4 py-2 rounded-soft shadow-lg text-sm">
          {toast}
        </div>)}

      {takeBrigade && villageId && (<BrigadeTakeDialog brigade={takeBrigade} teamId={teamId} onClose={() => setTakeBrigade(null)} onSuccess={async () => {
                setTakeBrigade(null);
                if (villageId)
                    await refresh(villageId);
            }}/>)}
...
function BrigadeTakeDialog({ brigade, teamId, onClose, onSuccess }) {
    ...
    const toggle = (id) => {
        ...
        if (next.has(id))
            next.delete(id);
        else if (next.size < brigade.required_player_count)
            next.add(id);
        setSelected(next);
    };
    const submit = async () => {
        if (selected.size !== brigade.required_player_count || submitting)
            return;
        setSubmitting(true);
        setErrMsg(null);
        try {
    ...
    return (<Modal isOpen onClose={onClose} maxWidth="640px">
      <div className="p-5 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3 text-sm bg-gray-50 rounded-soft p-3">
          ...
          <div>
            ...
            {brigade.morale_change !== 0 && (<div className={`text-xs ${brigade.morale_change > 0 ? "text-pitch-600" : "text-card-red"}`}>
        ...
        <div>
          ...
          {loading ? (<Spinner />) : eligible.length < brigade.required_player_count ? (<div className="text-sm text-card-red">
              Nemáš dost způsobilých hráčů (potřeba ≥ 60 kondice, bez zranění).
            </div>) : (<div className="max-h-72 overflow-y-auto border rounded-soft divide-y divide-gray-100">
              {eligible.map((p) => {
                let lc = {};
                try {
                    lc = JSON.parse(p.life_context);
                ...
                return (<label key={p.id} className={`flex items-center gap-3 px-3 py-2 cursor-pointer ${disabled ? "opacity-40 cursor-not-allowed" : "hover:bg-gray-50"}`}>
                    <input type="checkbox" checked={isSelected} onChange={() => toggle(p.id)} disabled={disabled}/>
                    <div className="flex-1 min-w-0">
        ...
        <div className="flex justify-end gap-2">
          ...
          <Button onClick={submit} disabled={selected.size !== brigade.required_player_count || submitting}>
