...
import { ROLE_DEFS, STAFF_ROLE_ORDER, STAFF_ATTRIBUTE_LABELS, STAFF_GROUP_LABELS, staffAttributeValue, staffEffectiveness, } from "@okresni-masina/shared";
// Pořadí určuje i výchozí záložku — první je ta bez ?tab= v adrese.
const TAB_KEYS = ["team", "market"];
const ATTR_ORDER = [
...
const GROUP_ORDER = ["trenerske", "zdravi", "provoz", "scouting"];
function czk(n) { return `${n.toLocaleString("cs")} Kč`; }
function attrColor(v) {
    if (v >= 15)
        return "text-pitch-400 font-bold";
    if (v >= 11)
...
function AttrGrid({ m }) {
    return (<div className="grid grid-cols-7 gap-1.5">
      {ATTR_ORDER.map((a) => {
            ...
            return (<div key={a} className="text-center">
...
function Avatar({ m, size = 48 }) {
    const hasFace = m.avatar && typeof m.avatar === "object" && Object.keys(m.avatar).length > 2;
    return hasFace
        ? <FaceAvatar faceConfig={m.avatar} size={size} className="rounded-soft bg-white"/>
...
export default function ZamestnanciPage() {
    ...
    const [marketRoleFilter, setMarketRoleFilter] = useState("all");
    const refresh = async () => {
        if (!teamId)
            return;
        const [s, m] = await Promise.all([
            apiFetch(`/api/teams/${teamId}/staff`).catch((e) => { console.error("load staff:", e); return null; }),
            apiFetch(`/api/teams/${teamId}/staff/market`).catch((e) => { console.error("load market:", e); return null; }),
        ]);
        if (s)
            setHired(s.staff ?? []);
        if (m)
    ...
    const tabs = [["team", "Tým", hired.length], ["market", "Volní", market.length]];
    const filteredMarket = marketRoleFilter === "all" ? market : market.filter((c) => c.profession === marketRoleFilter);
    return (<>
      <div className="page-container space-y-4">
        {/* Tab bar */}
        <Tabs value={tab} onChange={setTab} ariaLabel="Zaměstnanci" items={tabs.map(([key, label, count]) => ({ key, label, count: count || null }))}/>
                ...
                return (<div key={group} className="space-y-2">
                  ...
                  <div className="grid gap-3 sm:grid-cols-2">
                    {rolesInGroup.map((role) => {
                        ...
                        if (!m) {
                            // Prázdný slot
                            return (<div key={role} className="card p-4 border-2 border-dashed border-gray-100 flex flex-col gap-1.5">
                            ...
                            <button onClick={() => setTab("market")} className="btn btn-primary btn-sm self-start mt-1">Najmout →</button>
                          </div>);
                        }
                        const eff = staffEffectiveness(m, role);
                        return (<div key={role} className="card p-4 space-y-3">
                          ...
                          {/* Kurz */}
                          {m.courseAttribute ? (<div className="text-xs bg-gold-50 text-gold-700 rounded-soft px-3 py-2 font-heading">
                            ...
                            </div>) : m.courses && m.courses.length > 0 ? (<details className="text-xs">
                              <summary className="cursor-pointer text-muted hover:text-ink font-heading">📚 Poslat na kurz…</summary>
                              <div className="mt-2 grid grid-cols-1 gap-1.5">
                                {m.courses.map((q) => (<button key={q.attribute} onClick={() => doCourse(m, q)} className="flex items-center justify-between gap-2 text-left rounded-soft border border-gray-100 px-2.5 py-1.5 hover:border-pitch-300 transition-colors">
                          ...
                          <div className="flex items-center justify-between border-t border-gray-50 pt-2.5">
                            ...
                            <div className="flex items-center gap-2">
                              <select value="" onChange={(e) => { const r = e.target.value; if (r)
                            doReassign(m, r); }} className="text-xs border border-gray-200 rounded-soft px-2 py-1 font-heading text-muted bg-white">
                                <option value="">Přeřadit…</option>
                                {STAFF_ROLE_ORDER.filter((r) => r !== role && !occupiedRoles.has(r)).map((r) => (<option key={r} value={r}>{ROLE_DEFS[r].label} ({staffEffectiveness(m, r)}/20)</option>))}
                              </select>
                              <button onClick={() => doFire(m)} className="text-xs text-muted hover:text-card-red font-heading uppercase transition-colors">✕ propustit</button>
                            </div>
                          </div>
                        </div>);
                    })}
        ...
        {tab === "market" && (<div className="space-y-3">
            {market.length === 0 ? (<div className="card p-8 text-center text-muted">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <select value={marketRoleFilter} onChange={(e) => setMarketRoleFilter(e.target.value)} className="text-sm border border-gray-200 rounded-soft px-2.5 py-1.5 font-heading bg-white flex-1 min-w-[10rem]">
                ...
                {filteredMarket.length === 0 ? (<div className="card p-6 text-center text-muted text-sm">
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <select value={selected} onChange={(e) => setPickRole((p) => ({ ...p, [cand.id]: e.target.value }))} className="text-xs border border-gray-200 rounded-soft px-2 py-1 font-heading bg-white flex-1 min-w-[10rem]">
                        {STAFF_ROLE_ORDER.map((r) => (<option key={r} value={r} disabled={occupiedRoles.has(r)}>
                    ...
                    <div className="flex items-center justify-between border-t border-gray-50 pt-2.5">
                      ...
                      <button onClick={() => doHire(cand, selected)} disabled={occupiedRoles.has(selected)} className="btn btn-primary btn-sm disabled:opacity-40 disabled:cursor-not-allowed">
                        Najmout
                      </button>
                    </div>
                  </div>);
