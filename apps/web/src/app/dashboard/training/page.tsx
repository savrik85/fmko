...
const TRAINING_TYPES: Array<{ key: TrainingType; label: string; icon: string; desc: string; skills: string }> = [
  { key: "conditioning", label: "Kondice", icon: "\u{1F3C3}", desc: "Fyzická příprava", skills: "Výdrž, Rychlost, Síla" },
  { key: "technique", label: "Technika", icon: "\u26BD", desc: "Práce s míčem", skills: "Technika, Střelba, Přihrávky" },
  { key: "tactics", label: "Taktika", icon: "\u{1F9E0}", desc: "Herní systém", skills: "Přihrávky, Obrana + Chemie" },
  { key: "match_practice", label: "Zápasová praxe", icon: "\u{1F3AF}", desc: "Modelové situace", skills: "Střelba, Technika, Hlavičky" },
];
...
interface TrainingResult {
  attendance: Array<{ playerName: string; attended: boolean; reason?: string }>;
  improvements: Array<{ playerName: string; attribute: string; change: number }>;
                ...
                <div className="space-y-1">
                  {result.improvements.filter((i) => i.change > 0).map((imp, idx) => (
                    <div key={idx} className="flex items-center gap-2 text-sm">
                      ...
                      <span className="font-medium">{imp.playerName}</span>
