...
export async function generateMetadata({ params }) {
    ...
    const club = await fetchSafe(`/api/teams/${teamId}/club`);
    if (!club)
        return { title: "Profil klubu. Prales" };
...
export default async function KlubPublicPage({ params }) {
    const { teamId } = await params;
    const [team, club, manager, standings] = await Promise.all([
        fetchSafe(`/api/teams/${teamId}`),
        fetchSafe(`/api/teams/${teamId}/club`),
        fetchSafe(`/api/teams/${teamId}/manager`),
        fetchSafe(`/api/teams/${teamId}/standings`),
    ]);
    if (!club || !team) {
        return (<main className="min-h-dvh flex items-center justify-center bg-[#0a0f0a] text-white">
    ...
    const badgePattern = club.badge.pattern || "shield";
    ...
    const presentAttrs = manager
        ? ["coaching", "motivation", "tactics", "youthDevelopment", "discipline", "reputation"]
            .map((k) => ({ key: k, value: manager[k] }))
            .filter((a) => a.value != null)
        : [];
    return (<main className="min-h-dvh bg-[#0a0f0a] text-white">
        ...
        <div className="relative max-w-[1200px] mx-auto px-5 sm:px-10 pt-16 sm:pt-24 pb-24 sm:pb-32">
          ...
          <div className="flex flex-col sm:flex-row items-center sm:items-end gap-8 sm:gap-12">
            <div className="shrink-0 relative">
              <div className="absolute inset-0 rounded-full blur-3xl opacity-40" style={{ background: secondary, transform: "scale(1.5)" }}/>
            ...
            <div className="flex-1 min-w-0 text-center sm:text-left">
              {club.identity.nickname && (<div className={`text-xs sm:text-sm font-heading font-bold uppercase tracking-[0.25em] ${heroSoft} mb-2`}>
              ...
              {club.identity.motto && (<div className={`${heroText} italic text-xl sm:text-3xl mt-6 opacity-95 font-heading`}>
          ...
          {(() => {
            const chips = [];
            if (myPos && standings?.leagueName)
                chips.push({ label: "Pozice", value: `${myPos}.`, sub: standings.leagueName });
            if (club.identity.foundingYear)
                chips.push({ label: "Založeno", value: String(club.identity.foundingYear), sub: `${new Date().getFullYear() - club.identity.foundingYear} let existence` });
            if (club.stadium.capacity)
                chips.push({ label: "Kapacita", value: club.stadium.capacity.toLocaleString("cs"), sub: club.stadium.name || "Stadion" });
            if (club.village.population)
                chips.push({ label: "Vesnice", value: club.village.population.toLocaleString("cs"), sub: "obyvatel" });
            if (chips.length === 0)
                return null;
            return (<div className="mt-10 sm:mt-16 grid grid-cols-2 sm:grid-cols-4 gap-3">
                {chips.map((c, i) => {
                    const isLastOdd = chips.length % 2 === 1 && i === chips.length - 1;
                    return (<div key={c.label} className={`${chipBg} border ${chipBorder} backdrop-blur-sm rounded-2xl px-5 py-4 ${isLastOdd ? "col-span-2 sm:col-span-1" : ""}`}>
      ...
      <section className="relative py-10 sm:py-14">
        <div className="max-w-[1200px] mx-auto px-5 sm:px-10">
          <div className="w-full sm:w-4/5 mx-auto">
            <ClubScarf primary={club.badge.primary} secondary={club.badge.secondary} pattern={badgePattern} scarfPattern={club.scarfPattern || "classic"} initials={badgeIni} symbol={club.badge.symbol} className="h-24 sm:h-36 w-full"/>
          </div>
        </div>
      </section>
      ...
      <section className="relative py-16 sm:py-24 overflow-hidden">
        <div className="max-w-[1200px] mx-auto px-5 sm:px-10">
          ...
          <div className="relative rounded-[2.5rem] p-8 sm:p-16 overflow-hidden" style={{ background: `linear-gradient(135deg, ${primary}15, transparent 50%, ${secondary}10), #13191c` }}>
            ...
            <div className="relative flex items-end justify-center gap-6 sm:gap-20 flex-wrap" style={{ filter: "drop-shadow(0 25px 40px rgba(0,0,0,0.5))" }}>
              ...
              <div className="flex flex-col items-center self-center order-first sm:order-none">
                ...
                {club.jersey.sponsor && (<div className="mt-6 bg-white text-black font-heading font-bold uppercase tracking-wider text-xs px-4 py-2 rounded shadow-lg">
                    {club.jersey.sponsor}
                  </div>)}
              </div>

              {/* Away */}
              {(club.jersey.awayPrimary || club.jersey.awaySecondary) && (<div className="flex flex-col items-center">
        ...
        <div className="max-w-[1200px] mx-auto px-5 sm:px-10">
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-6 lg:gap-10 items-start">
            <div className="lg:col-span-2">
              ...
              {club.stadium.nickname && (<div className="italic text-xl sm:text-2xl mt-3 text-white/60">&ldquo;{club.stadium.nickname}&rdquo;</div>)}
              {club.stadium.specialita && (<div className="mt-6 p-5 rounded-2xl border border-white/10 bg-white/5">
            ...
            <div className="lg:col-span-3 grid grid-cols-2 gap-3 sm:gap-4">
              {(() => {
            const cards = [];
            if (club.stadium.capacity != null)
                cards.push({ label: "Kapacita", value: club.stadium.capacity.toLocaleString("cs"), big: true });
            if (club.stadium.builtYear != null)
                cards.push({ label: "Postaveno", value: String(club.stadium.builtYear), big: true });
            if (club.stadium.tribunaNorth)
                cards.push({ label: "Severní tribuna", value: club.stadium.tribunaNorth });
            if (club.stadium.tribunaSouth)
                cards.push({ label: "Jižní tribuna", value: club.stadium.tribunaSouth });
            return cards.map((c, i) => {
                const isLastOdd = cards.length % 2 === 1 && i === cards.length - 1;
                return (<div key={c.label} className={`p-5 sm:p-6 rounded-2xl border border-white/10 bg-gradient-to-br from-white/5 to-transparent ${isLastOdd ? "col-span-2" : ""}`}>
              ...
              {club.stadium.namingSponsor && (<div className="col-span-2 p-5 rounded-2xl border border-yellow-500/30 bg-yellow-500/5">
      ...
      {/* ═══ HYMNA ═══ */}
      {club.anthem.url && (<section className="py-16 sm:py-24 relative overflow-hidden">
          <div className="absolute inset-0 opacity-30" style={{
          ...
          <div className="relative max-w-[1000px] mx-auto px-5 sm:px-10">
            ...
            <div className="rounded-3xl border border-white/10 bg-gradient-to-b from-white/5 to-transparent backdrop-blur p-6 sm:p-10">
              ...
              {club.anthem.lyrics && (<details className="mt-6 group">
      ...
      {/* ═══ MASKOT ═══ */}
      {club.mascot.imageUrl && (<section className="py-16 sm:py-24">
          <div className="max-w-[1200px] mx-auto px-5 sm:px-10">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16 items-center">
              <div className="order-2 lg:order-1">
                ...
                {club.mascot.story && (<p className="text-lg sm:text-xl text-white/80 mt-6 italic leading-relaxed">&ldquo;{club.mascot.story}&rdquo;</p>)}
              </div>
      ...
      {/* ═══ TRENÉR ═══ */}
      {manager && (<section className="py-16 sm:py-24">
          <div className="max-w-[1200px] mx-auto px-5 sm:px-10">
              ...
              <div className="flex flex-col sm:flex-row gap-8 sm:gap-12 items-center sm:items-start">
                <div className="shrink-0 relative">
                  ...
                  <div className="relative rounded-3xl overflow-hidden border-2 border-white/20 bg-white/5" style={{ width: 160, height: 192 }}>
                    {manager.avatar ? (<ManagerFace faceConfig={manager.avatar} size={160}/>) : (<div className="w-full h-full flex items-center justify-center text-6xl text-white/30">{"\u{1F464}"}</div>)}
                  </div>
                </div>
                <div className="flex-1 min-w-0 text-center sm:text-left">
                  ...
                  {manager.bio && (<p className="text-white/80 text-base sm:text-lg mt-5 italic leading-relaxed max-w-2xl">&ldquo;{manager.bio}&rdquo;</p>)}

                  {presentAttrs.length > 0 && (<div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mt-8 max-w-2xl">
                      {presentAttrs.map(({ key, value }, i) => {
                    const isLastOdd = presentAttrs.length % 2 === 1 && i === presentAttrs.length - 1;
                    return (<div key={key} className={`bg-white/5 rounded-xl px-4 py-3 border border-white/5 ${isLastOdd ? "col-span-2 sm:col-span-1" : ""}`}>
                            ...
                            <div className="flex items-center gap-2 mt-1">
                              <div className="flex-1 h-1.5 bg-white/10 rounded-full overflow-hidden">
                                <div className="h-full rounded-full" style={{ width: `${Math.min(100, value)}%`, background: `linear-gradient(90deg, ${primary}, ${secondary})` }}/>
                              </div>
      ...
      {/* ═══ PŘÍBĚH KLUBU ═══ */}
      {(club.identity.foundingStory || club.identity.colorsMeaning) && (<section className="py-16 sm:py-24">
          <div className="max-w-[900px] mx-auto px-5 sm:px-10">
            ...
            {club.identity.foundingStory && (<p className="text-lg sm:text-xl leading-relaxed whitespace-pre-line text-white/85">
                {club.identity.foundingStory}
              </p>)}
            {club.identity.colorsMeaning && (<div className="mt-10 pt-10 border-t border-white/10">
