...
export default async function Image({ params }) {
    ...
    return new ImageResponse((<div style={{
        ...
        <div style={{
          ...
          {badgeSymbol && (<div style={{ fontSize: 90, marginTop: -10, display: "flex" }}>{badgeSymbol}</div>)}
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", flex: 1, minWidth: 0 }}>
          ...
          {nickname && (<div style={{ fontSize: 36, fontWeight: 700, color: subColor, marginBottom: 12 }}>
              {nickname}
            </div>)}
          {(village || district) && (<div style={{ fontSize: 26, color: subColor, marginBottom: motto ? 20 : 0 }}>
              {village}{district ? ` · ${district}` : ""}
            </div>)}
          {motto && (<div style={{ fontSize: 28, fontStyle: "italic", color: subColor, marginTop: 14, display: "flex" }}>
