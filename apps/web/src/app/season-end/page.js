...
export default function SeasonEndPage() {
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                const d = await apiFetch(`/api/teams/${me.teamId}/season-recap`);
                if (cancelled)
                    return;
    ...
    const finish = async (leaving) => {
        const teamId = recap?.__teamId;
        if (teamId) {
...
// ═══ Recap experience ═══
function moodFor(pos, total) {
    if (pos === 1)
        return { kind: "champion", kicker: "MISTR LIGY", accent: "#F0D060", confetti: 140 };
    if (pos != null && pos <= 3)
        return { kind: "podium", kicker: "STUPNĚ VÍTĚZŮ", accent: "#F0D060", confetti: 80 };
    if (pos != null && pos <= Math.ceil(total / 2))
        return { kind: "solid", kicker: "SOLIDNÍ SEZÓNA", accent: "#4CAF50", confetti: 0 };
    return { kind: "tough", kicker: "TĚŽKÁ SEZÓNA", accent: "#D94032", confetti: 0 };
}
function Recap({ data, onEnter }) {
    const team = data.primaryColor || "#2D5F2D";
    const myTeamId = data.__teamId;
    ...
    return (<div className="se-root" style={{ ["--team"]: team, ["--accent"]: mood.accent }}>
      <style>{CSS}</style>

      {/* atmosféra */}
      <div className="se-glow"/>
      <div className="se-grain"/>
      {confetti.length > 0 && (<div className={`se-confetti${confettiOn ? "" : " gone"}`} aria-hidden>
          {confetti.map((c, i) => (<span key={i} style={{ left: `${c.left}%`, width: c.size, height: c.size * 1.6, background: c.color, animationDelay: `${c.delay}s`, animationDuration: `${c.dur}s`, ["--rot"]: `${c.rot}deg` }}/>))}
        </div>)}

      <div className="se-scroll" onScroll={(e) => { if (confettiOn && e.currentTarget.scrollTop > window.innerHeight * 0.4)
        setConfettiOn(false); }}>
        {/* 1 — HERO */}
        <Section className="se-hero">
          <div className="se-kicker">{data.leagueName} · Sezóna {data.seasonNumber}</div>
          {mood.kind === "champion" ? (<div className="se-champhero">
              ...
              {data.finalPos != null ? (<div className="se-placement">
                ...
                </div>) : (<div className="se-placement"><span className="se-pos" style={{ color: mood.accent }}>—</span></div>)}
        ...
        <Section className="se-block">
          ...
          {data.awards.bestEleven.length > 0 && (<div className="se-xi">
              ...
              <div className="se-xi-grid">
                ...
                return (<div key={p.playerId} className={`se-xi-chip${mine ? " mine" : ""}`}>
        ...
        {duels.length > 0 ? (<Section className="se-block">
            ...
            <div className="se-duels">
              {duels.map((d, i) => (<div key={i} className="se-duel-wrap">
          ...
          </Section>) : (<Section className="se-block">
            <h2 className="se-h2">Rozloučili jsme se</h2>
            {data.departures.length === 0 ? (<p className="se-empty">Kádr zůstal pohromadě — nikdo nepověsil kopačky na hřebík.</p>) : (<div className="se-deps">
                {data.departures.map((d, i) => (<div key={i} className="se-dep">
        ...
        {/* 4b — VÝVOJ */}
        {(data.manager || data.playerDev.improved.length > 0 || data.playerDev.declined.length > 0) && (<Section className="se-block">
            <h2 className="se-h2">Vývoj týmu</h2>
            {data.manager && (<div className="se-mgr">
                <div className="se-mgr-head">Trenér <strong>{data.manager.name}</strong> · {data.manager.age} let</div>
                {data.manager.deltas.length > 0 ? (<div className="se-mgr-deltas">
                  ...
                  </div>) : <div className="se-mgr-flat">Trenér beze změny atributů.</div>}
              </div>)}
            <div className="se-dev-grid">
              {data.playerDev.improved.length > 0 && (<div className="se-dev-col">
                  <div className="se-dev-title up">📈 Největší zlepšení</div>
                  {data.playerDev.improved.map((p) => <DevRow key={p.playerId} p={p}/>)}
                </div>)}
              {data.playerDev.declined.length > 0 && (<div className="se-dev-col">
        ...
        {(() => {
            ...
            if (!hasRel && !data.village)
                return null;
            return (<Section className="se-block">
              <h2 className="se-h2">Kabina &amp; rivalové</h2>
              <div className="se-rel-grid">
                {rel?.rival && (<div className="se-rel-card rival">
                ...
                {rel?.ally && (<div className="se-rel-card ally">
                ...
                {rel?.favorite && (<div className="se-rel-card">
                ...
                {rel?.blackSheep && (<div className="se-rel-card">
                ...
                {data.village && (<div className="se-rel-card village">
        ...
        {/* 4d — VÝROK SEZONY */}
        {data.quote && (<Section className="se-center">
        ...
        {/* 5 — TROFEJ */}
        {data.trophy && (<Section className="se-center">
        ...
        {data.party && (<Section className="se-pub">
            ...
            {data.party.awards.length > 0 && (<div className="se-pub-block">
                <div className="se-pub-label">🏅 Ceny hospodského</div>
                <div className="se-pub-awards">
                  {data.party.awards.map((a, i) => (<div key={i} className="se-pub-award">
            ...
            {data.party.scenes.length > 0 && (<div className="se-pub-block">
        ...
        {data.summer && data.summer.length > 0 && (<Section className="se-block">
            <h2 className="se-h2">Co se stalo přes léto 👷</h2>
            <div className="se-summer">
              {data.summer.map((s, i) => (<div key={i} className={`se-summer-row${s.effect ? ` ${s.effect}` : ""}`}>
...
function Section({ children, className = "" }) {
    ...
    useEffect(() => {
        ...
        const obs = new IntersectionObserver((entries) => entries.forEach((e) => { if (e.isIntersecting)
            el.setAttribute("data-in", "1"); }), { threshold: 0.25 });
        obs.observe(el);
...
function RewardCounter({ value }) {
    ...
    return (<div ref={ref} className="se-reward">
      <span className="se-reward-plus">+</span>{n.toLocaleString("cs")}<span className="se-reward-cur"> Kč</span>
    </div>);
}
function AwardCard({ icon, label, a, extra, mine }) {
    if (!a?.name)
        return null;
    return (<div className={`se-award${mine ? " mine" : ""}`}>
...
function DevRow({ p }) {
    const up = p.delta > 0;
    return (<div className="se-dev-row">
...
function SpeechPicker({ teamId, initial }) {
    ...
    return (<div className="se-pub-block">
      <div className="se-pub-label">🎤 Proslov trenéra</div>
      {!picked ? (<div className="se-speech-opts">
          {SPEECH_TONES.map((t) => (<button key={t.key} type="button" className="se-speech-btn" onClick={() => choose(t.key)}>
              <span className="se-speech-label">{t.label}</span>
              <span className="se-speech-desc">{t.desc}</span>
            </button>))}
        </div>) : (<p className="se-speech-outcome">„{chosen?.outcome}"</p>)}
    </div>);
}
function DuelCard({ p, state, onPick }) {
    return (<button type="button" className={`se-duelcard ${state}`} onClick={onPick}>
