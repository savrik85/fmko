...
export default function VudceDetailPage() {
  ...
  return (
    <div className="page-container pb-24">
      <Link href="/fanousci?tab=groups" className="text-sm text-muted hover:underline">← Fanoušci</Link>
      ...
      {data.team && (
        <p className="text-sm text-muted mt-4">
          Chodí na{" "}
          <Link href="/muj-klub" className="underline font-medium">{data.team.name}</Link>.
        </p>
      )}
    </div>
  );
