      ...
      >
        ...
        {onZoom && (
          <button
            ...
            className="absolute top-3 right-3 w-9 h-9 rounded-soft bg-white/80 hover:bg-white text-gray-700 flex items-center justify-center text-base shadow transition-colors"
          >
            {"\u{1F50D}"}
          </button>
        )}
  ...
  return (
    <label className="flex items-center gap-3">
      <input
        ...
        className="w-12 h-12 rounded-soft cursor-pointer border-2 border-gray-200 shrink-0"
        aria-label={label}
      />
      ...
      {options.map((opt) => {
        ...
        return (
          <button
            ...
            className={`flex flex-col items-center gap-1.5 p-2 rounded-soft border-2 transition-all ${
...
export default function DresPage() {
      ...
      <CardBody className="flex flex-col gap-4">
        ...
        <div>
          ...
          <input
            ...
            maxLength={5}
            className="w-full px-3 py-2 border border-gray-200 rounded-soft text-sm font-mono uppercase focus:border-pitch-500 focus:outline-none"
          />
            ...
            {BADGE_SYMBOLS.map((opt) => {
              ...
              return (
                <button
                  ...
                  title={opt.label}
                  className={`aspect-square flex items-center justify-center text-xl rounded-soft border-2 transition-all ${
                    active ? "border-pitch-500 bg-pitch-50" : "border-gray-200 hover:border-gray-300 bg-white"
                  }`}
                >
  ...
  return (
    <div className="page-container">
      ...
      <div className="mt-4 p-3 rounded-soft bg-gray-50 border border-gray-200 text-sm">
        ...
        <Link href="/sponzori" className="text-pitch-600 underline hover:text-pitch-700">Sponzoři</Link>.
      </div>
      ...
      <StickyActions>
        ...
        <button
          type="button"
          onClick={() => router.push("/muj-klub")}
          className="px-4 py-2 rounded-soft text-sm font-heading font-bold text-muted hover:text-ink"
        >
          Zrušit
        </button>
        <button
      ...
      <Modal isOpen={zoomedKit !== null} onClose={() => setZoomedKit(null)} maxWidth="760px">
        <div className="p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <button type="button" onClick={() => setZoomedKit(null)} className="w-8 h-8 rounded-soft text-gray-400 hover:text-ink hover:bg-gray-100 text-xl">×</button>
          </div>
