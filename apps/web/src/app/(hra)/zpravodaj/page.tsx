...
function ShareButton({ articleId }: { articleId: string }) {
  ...
  const handleShare = async () => {
    ...
    const url = `${window.location.origin}/zpravodaj?article=${articleId}`;
                ...
                {postMatchArticles.map((iv) => {
                  ...
                  return (
                    <Rozhovor
                      ...
                      jmenoHref={(meta.teamId ?? iv.teamId) ? `/manazer/${meta.teamId ?? iv.teamId}` : undefined}
                      ...
                      poznamkaHref={meta.refereeId ? `/rozhodci/${meta.refereeId}` : undefined}
                    />
                  );
                })}
              </div>
                ...
                {currentWeekInterviews.map((iv) => {
                  ...
                  return (
                    <Rozhovor
                      ...
                      tym={meta.teamName}
                      avatar={meta.managerAvatar}
                      jmenoHref={iv.teamId ? `/manazer/${iv.teamId}` : undefined}
                      text={meta.article ?? iv.body}
...
function ArticleWrapper({ article, children }: { article: Article; children: React.ReactNode }) {
  if (article.type === "match") {
    return <Link href={`/zapas/${article.id}`} className="block">{children}</Link>;
  }
