    ...
    apiFetch<MatchDetail & { calendar_id?: string | null }>(`/api/matches/${matchId}`)
      .then((m) => {
        ...
        if (m.status !== "simulated") {
          ...
          router.replace(`/zapas?calendarId=${calId}`);
          return;
        }
        setMatch(m);
        setLoading(false);
      ...
      <div className="rounded-xl overflow-hidden" style={{ background: "#0a0a0a" }}>
        ...
        <div className="flex items-center px-4 pt-3">
          ...
          {match.status === "simulated" && match.events.length > 0 && (
            <button onClick={() => router.push(`/zapas/${matchId}/zaznam`)}
        ...
        <div className="flex flex-wrap items-center justify-center gap-x-4 gap-y-1 px-4 py-2 text-white/50 text-xs sm:text-sm font-heading" style={{ background: "#060d06" }}>
          ...
          {refereeInfo && (
            <span>
              Rozhodčí:{" "}
              {refereeInfo.id ? (
                <Link href={`/rozhodci/${refereeInfo.id}`} className="text-white/80 hover:underline">{refereeInfo.name}</Link>
              ) : (
          ...
          <div className="card p-4">
            <div className="flex items-center justify-between gap-3 flex-wrap">
              <div className="min-w-0">
                {refereeInfo.id ? (
                  <Link href={`/rozhodci/${refereeInfo.id}`} className="font-heading font-bold text-base hover:underline">
                    {refereeInfo.name}
                  </Link>
                ) : (
          ...
          <div className="rounded-xl overflow-hidden border border-gold-500/30 shadow-md" style={{ background: `linear-gradient(135deg, ${teamColor}, color-mix(in srgb, ${teamColor} 70%, black))` }}>
            ...
            <div className="p-4 sm:p-5 flex items-center gap-4">
              ...
              <div className="flex-1 min-w-0">
                <Link href={`/hrac/${mvp.id}`} className={`block font-heading font-[800] text-xl sm:text-2xl truncate hover:underline ${textOnBg}`}>
                  {mvp.player.name}
                </Link>
                <div className={`text-sm ${subTextOnBg} flex items-center gap-1.5 flex-wrap`}>
                  <Link href={`/tym/${teamId}`} className="hover:underline font-heading font-bold">{teamName}</Link>
                  <span>·</span>
                ...
                {top.length === 0 ? (
                  ...
                  return (
                    <div key={entry.id} className="flex items-center gap-3 px-3 py-2">
                      <div className="flex-1 min-w-0">
                        <Link href={`/hrac/${entry.id}`} className="block font-heading font-bold text-base hover:text-pitch-500 transition-colors truncate">
                          {entry.player!.name}
                        </Link>
                      ...
                      {g.players.map((p) => {
                        ...
                        <div key={p.id} className={`flex items-center gap-2 px-3 py-1.5 border-l-3 ${rowBorderClass} ${isMvp ? "bg-gold-50/50 ring-1 ring-inset ring-gold-300" : ""}`}>
                          ...
                          <span className="flex-1 min-w-0 truncate text-base leading-6">
                            ...
                            {p.id ? (
                              <Link href={`/hrac/${p.id}`} className="font-heading font-bold hover:text-pitch-500 transition-colors">{p.name}</Link>
                            ) : (
          ...
          <div className="card p-3 sm:p-4 bg-gray-50 border-gray-200">
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <Link href={`/hrac/${flop.id}`} className="block font-heading font-bold text-base hover:text-pitch-500 transition-colors truncate">
                  {flop.player.name}
                </Link>
                <div className="text-xs text-muted">
                  <Link href={`/tym/${teamId}`} className="hover:underline">{teamName}</Link>
