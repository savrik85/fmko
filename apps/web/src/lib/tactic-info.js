...
export function getTacticTooltip(key) {
    const info = TACTIC_INFO[key];
    if (!info)
        return "";
    return `${info.label} ${info.description}`;
}
export function getFormationTooltip(key) {
    const info = FORMATION_INFO[key];
    if (!info)
        return key;
    return `${info.label} ${info.description}`;
}
export const HARDNESS_INFO = {
    fair: {
        label: "Na férovku",
        icon: "🤝",
...
export function getHardnessTooltip(key) {
    const info = HARDNESS_INFO[key];
    if (!info)
        return "";
    return `${info.label} ${info.description}`;
}
//# sourceMappingURL=tactic-info.js.map