 ...
 */
export function generateCharacteristics(player, teamPlayers, maxTags = 3, 
...
jeDorost = false) {
    ...
    if (player.is_celebrity) {
        const celebType = String(player.personality?.celebrityType ?? "legend");
        if (celebType === "fallen_star") {
            ...
            const tierLabel = { S: "Legenda", A: "Ex-ligista", B: "Druholigový matador", C: "Krajský borec" };
            const t = String(player.personality?.celebrityTier ?? "C");
