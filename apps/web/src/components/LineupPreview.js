...
export function LineupPreview({ teamId, matchId, formation, tactic, hardness, captainId, players, onCardRisk }) {
    ...
    useEffect(() => {
        ...
        debounceRef.current = setTimeout(() => {
            setLoading(true);
            apiFetch(`/api/teams/${teamId}/lineup-preview`, {
                method: "POST",
    ...
    return (<div className="card p-3">
      <button onClick={() => setCollapsed((c) => !c)} className="w-full flex items-center justify-between text-left mb-2">
        <div>
      ...
      {!collapsed && data && (<>
          ...
          <div className="space-y-1.5 mt-2">
            {Object.keys(LINE_LABELS).map((line) => {
                ...
                return (<div key={line}>
                  ...
                  <div className="flex gap-1 items-center h-3">
                    ...
                    {oppVal !== undefined && (<div className="flex-1 bg-gray-100 rounded-full overflow-hidden">
          ...
          <div className="mt-3 pt-3 border-t border-gray-100 grid grid-cols-2 gap-2 text-micro">
            <div className="flex items-center gap-2">
              ...
              {data.own.tacticEffect.attackMod !== 1.0 && (<span className={`text-micro ${data.own.tacticEffect.attackMod > 1.0 ? "text-pitch-500" : "text-card-red"}`}>
            ...
            <div className="flex items-center gap-2">
              ...
              {data.own.tacticEffect.defenseMod !== 1.0 && (<span className={`text-micro ${data.own.tacticEffect.defenseMod > 1.0 ? "text-pitch-500" : "text-card-red"}`}>
          ...
          {data.recommendation && (<div className="mt-3 px-3 py-2 rounded-soft bg-pitch-50 border border-pitch-200 text-[12px] text-pitch-800">
              <span className="font-heading font-bold mr-1">💡</span>
              {data.recommendation}
            </div>)}
          {data.own.notes.length > 0 && (<ul className="mt-2 space-y-1 text-micro text-amber-700">
              {data.own.notes.map((note, i) => (<li key={i} className="flex items-start gap-1.5">
                  <span>⚠️</span>
                  <span>{note}</span>
                </li>))}
            </ul>)}
...
//# sourceMappingURL=LineupPreview.js.map