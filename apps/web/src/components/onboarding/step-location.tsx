...
function getDifficultyStars(category: string): string {
  switch (category) {
    case "vesnice": return "\u2B50\u2B50\u2B50 Hardcore";
    case "obec": return "\u2B50\u2B50 Výzva";
    case "mestys": return "\u2705 Dobrý start";
    case "mesto": return "\u{1F7E2} Easy mode";
    default: return "";
  }
}
...
export function StepLocation({ onSelect }: Props) {
  ...
  const regions = useMemo(() =>
    [...new Map(villages.map((v) => [v.region_code, v.region])).entries()]
  ...
  const districts = useMemo(() => {
    ...
    return [...new Map(filtered.map((v) => [v.district_code, v.district])).entries()]
      .map(([code, name]) => ({ code, name }))
      .sort((a, b) => a.name.localeCompare(b.name, "cs"));
  }, [selectedRegion]);
        ...
        {filteredVillages.slice(0, 50).map((v) => (
          ...
          >
            ...
            <div className="text-xs text-right">
              <div>{getDifficultyStars(v.category)}</div>
