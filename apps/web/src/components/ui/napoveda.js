...
function findHelp(pathname) {
    ...
    if (pathname.match(/\/dashboard\/match\/[^/]+$/))
        return HELP["/dashboard/match"];
    ...
    if (pathname.match(/\/dashboard\/phone\/.+/))
        return null;
    return null;
}
    ...
    return (<>
      ...
      <div className="fixed bottom-[5.5rem] sm:bottom-auto sm:top-16 left-3 right-3 sm:left-auto sm:right-4 z-[var(--z-nav)] sm:w-[360px] max-h-[55vh] sm:max-h-[70vh] overflow-y-auto overscroll-contain rounded-xl bg-white shadow-2xl border border-gray-100 animate-slide-up">
        ...
        <div className="px-5 py-4 space-y-3">
          {help.sections.map((s, i) => (<div key={i} className={s.tip ? "bg-pitch-50 border border-pitch-200 rounded-soft px-3 py-2" : ""}>
              {s.heading && (<h4 className="font-heading font-bold text-sm text-pitch-700 mb-0.5">{s.heading}</h4>)}
...
function HelpButton({ open, onClick, variant = "floating", className, }) {
    ...
    return (<button onClick={onClick} className={`${spolecne} ${styl}`} title={open ? "Zavřít nápovědu" : "Nápověda"} aria-label={open ? "Zavřít nápovědu" : "Nápověda"}>
      {open ? "✕" : "?"}
    </button>);
}
// ── Exports ────────────────────────────────────────────────
...
export function NapovedaOnboarding({ step }) {
    const [open, setOpen] = useState(false);
    const stepHelp = {
        location: {
            title: "Výběr obce",
...
//# sourceMappingURL=napoveda.js.map