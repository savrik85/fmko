...
export function DivergingBar({ data, positiveLabel, negativeLabel, }) {
    ...
    return (<div className="space-y-2">
      ...
      <ul className="space-y-1.5">
        {data.map((d) => (<li key={d.label} className="flex items-center">
            <span className={`${VALUE_COL} shrink-0 text-right text-sm tabular-nums text-muted pr-1`}>
              {d.negative > 0 ? d.negative : ""}
            </span>
            <span className="flex-1 flex justify-end">
              <span className="h-2 rounded-l-[4px] block" style={{ width: `${(d.negative / max) * 100}%`, background: DIVERGING.negative }} title={`${d.label} ${negativeLabel}: ${d.negative}`}/>
            </span>

            <span className={`${LABEL_COL} shrink-0 text-center text-sm truncate px-1.5`} style={{ borderLeft: `1px solid ${GRID}`, borderRight: `1px solid ${GRID}` }}>
              {d.label}
            </span>

            <span className="flex-1">
              <span className="h-2 rounded-r-[4px] block" style={{ width: `${(d.positive / max) * 100}%`, background: DIVERGING.positive }} title={`${d.label} ${positiveLabel}: ${d.positive}`}/>
            </span>
