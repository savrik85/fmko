...
export function TodayProgramWidget({ data }) {
    ...
    const isMatchDay = (() => {
        if (!nextMatch?.scheduledAt || !gameDate)
            return false;
    ...
    <div className={`flex items-center gap-4 ${isMatchDay ? "rounded-xl ring-2 ring-pitch-400 bg-pitch-50/30 p-3 mt-1" : ""}`}>
      ...
      <div className="flex-1">
        ...
        <div className="text-sm text-muted">
          ...
          {isMatchDay && matchOpponent && (<span> · <span className="font-heading font-bold text-pitch-600">{matchOpponent}</span> · výkop v 18:00</span>)}
          {isTrainingDay && !isMatchDay && <span> · Trénink dle plánu</span>}
          {!isTrainingDay && !isMatchDay && <span> · Regenerace, žádný program</span>}
        </div>
      </div>
      {isMatchDay && (<Link href="/dashboard/match" className="py-2 px-4 rounded-soft bg-pitch-500 text-white font-heading font-bold text-sm hover:bg-pitch-600 transition-colors">
          Sestava →
        </Link>)}
    </div>);
}
// ── Další zápas ─────────────────────────────────────────────────────────────
export function NextMatchWidget({ data, teamId }) {
    const team = data.team.data;
    ...
    if (data.schedule.loading || data.team.loading)
        return <WidgetSkeleton rows={5}/>;
    if (data.schedule.error)
        return <WidgetError />;
    if (!nextMatch || !team)
        return <div className="text-center text-muted py-4">Žádný naplánovaný zápas</div>;
    const gameDate = team.game_date ? new Date(team.game_date) : null;
    const color = team.primary_color || "#2D5F2D";
    const promoteMatch = async (m) => {
        if (!teamId || promoting)
            return;
        ...
        if (!ok)
            return;
        setPromoting(true);
        const res = await apiFetch(`/api/teams/${teamId}/matches/${m.id}/promote`, { method: "POST" }).catch((e) => { console.error("propagace zápasu:", e); return { error: "Chyba při propagaci" }; });
        setPromoting(false);
        if (res?.error) {
            showError("Chyba", res.error ?? "Zkus to prosím znovu.");
            return;
        }
        const refreshed = await apiFetch(`/api/teams/${teamId}/schedule`)
            .catch((e) => { console.error("obnovení rozpisu:", e); return null; });
        if (refreshed)
            setLocalMatches(refreshed.matches);
    };
    ...
    const oppBadge = (nextMatch.isHome ? nextMatch.awayBadge : nextMatch.homeBadge) || "shield";
    const myTeamData = {
        ...
        badge: team.badge_pattern || "shield",
    ...
    const TeamCell = ({ t, children }) => t.id
    ...
    return (<>
      ...
      <div className="overflow-hidden rounded-xl border border-gray-100">
          ...
          <div className="flex items-start gap-3">
            <TeamCell t={homeTeam}>
              <div className="flex justify-center mb-1.5">
                <BadgePreview primary={homeTeam.color} secondary={homeTeam.secondary} pattern={homeTeam.badge} initials={homeTeam.customInitials || initials(homeTeam.name)} symbol={homeTeam.symbol} size={38}/>
              </div>
              ...
              {homeTeam.pos && homeTeam.pos.position > 0 && (<div className="text-micro text-white/40 tabular-nums leading-tight">{homeTeam.pos.position}. místo</div>)}
            </TeamCell>
            ...
            <TeamCell t={awayTeam}>
              <div className="flex justify-center mb-1.5">
                <BadgePreview primary={awayTeam.color} secondary={awayTeam.secondary} pattern={awayTeam.badge} initials={awayTeam.customInitials || initials(awayTeam.name)} symbol={awayTeam.symbol} size={38}/>
              </div>
              <div className="font-heading font-bold text-sm leading-tight">{awayTeam.name}</div>
              {awayTeam.pos && awayTeam.pos.position > 0 && (<div className="text-micro text-white/40 tabular-nums leading-tight">{awayTeam.pos.position}. místo</div>)}
            </TeamCell>
          </div>
        </div>

        {homeForm && awayForm && (homeForm.form.length > 0 || awayForm.form.length > 0) && (<div className="flex items-center px-4 py-2 border-b border-gray-100 overflow-hidden">
        ...
        {preview && my && opp && (() => {
            ...
            return (<div className="px-4 py-3 space-y-2.5 border-b border-gray-100">
              {stats.map((s) => {
        ...
        {preview?.odds?.home && preview.odds.draw && preview.odds.away && (() => {
            ...
            const k = (x) => (x / 100).toFixed(2).replace(".", ",");
            const domaci = preview.odds.home, remiza = preview.odds.draw, hoste = preview.odds.away;
            const nej = Math.min(domaci, remiza, hoste);
            const bunka = (popis, kurz) => (<div className={`text-center py-1 rounded-tight ${kurz === nej ? "bg-pitch-50" : ""}`}>
            ...
            return (<div className="px-4 py-2.5 border-b border-gray-100">
        ...
        {preview && (<div className="flex items-center justify-between px-4 py-2.5 border-b border-gray-100">
            ...
            <Link href={nextMatch.isHome ? "/dashboard/stadium" : `/dashboard/team/${homeTeam.id}/stadium`} className="text-sm text-muted truncate ml-2 hover:text-pitch-600 hover:underline">
              {preview.venue.name}
            </Link>
          </div>)}
        ...
        <div className="text-center px-4 py-2">
          ...
          {nextMatch.isHome && nextMatch.promoted && (<span className="inline ml-4 text-sm text-gold-600 font-heading font-bold">📢 Propagováno</span>)}
          {nextMatch.isHome && !nextMatch.promoted && (<button onClick={() => promoteMatch(nextMatch)} disabled={promoting} className="inline ml-4 text-sm text-gold-600 font-heading font-bold hover:underline disabled:opacity-50">
...
function MiniForm({ f }) {
    return (<span className={`shrink-0 w-5 h-5 rounded-control text-micro flex items-center justify-center font-bold text-white ${f === "W" ? "bg-pitch-500" : f === "L" ? "bg-card-red" : "bg-gray-300"}`}>{f === "W" ? "V" : f === "L" ? "P" : "R"}</span>);
}
// ── Rozpis ──────────────────────────────────────────────────────────────────
export function FixturesWidget({ data, height }) {
    ...
    return (<>
      {upcoming.length > 0 ? (<div className="space-y-2">
          {upcoming.map((m) => {
                ...
                return (<Link key={m.id} href={`/dashboard/match?calendarId=${switchId}`} className="flex items-center gap-2 py-1.5 border-b border-gray-50 last:border-b-0 hover:bg-gray-50 -mx-2 px-2 rounded transition-colors">
        ...
        </div>) : (<div className="text-center text-muted py-4 text-sm">Žádné naplánované</div>)}
...
function LineupFlag({ m }) {
    ...
    if (m.hasLineup)
        return <span className="text-micro text-pitch-600" title="Sestava nastavena">✓</span>;
...
// ── Poslední zápasy ─────────────────────────────────────────────────────────
export function RecentMatchesWidget({ data, height }) {
    if (data.matchResults.loading)
        return <WidgetSkeleton />;
    if (data.matchResults.error)
        return <WidgetError />;
    const matches = data.matchResults.data?.matches ?? [];
    if (matches.length === 0)
        return <div className="text-sm text-muted py-4 text-center">Zatím žádný odehraný zápas.</div>;
    return (<>
      <div className="overflow-x-auto -mx-4 sm:-mx-5 table-scroll">
        <table className="w-full text-sm">
          ...
          <tbody>
            ...
            return (<tr key={m.id} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                  ...
                  <td className="py-2 pr-2">
                    <a href={`/dashboard/match/${m.id}`} className="flex items-center gap-2 hover:underline">
                      <BadgePreview primary={m.opponentColor} secondary={m.opponentSecondary} pattern={m.opponentBadge || "shield"} initials={initials(m.opponent ?? "")} size={20}/>
...
//# sourceMappingURL=match-widgets.js.map