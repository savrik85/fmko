...
- **Kde:** `equipment_listings` s `league_id` ligy okradeného klubu, `team_id = NULL`, `is_ai_listing = 0` (nesmí ubírat z cíle `TARGET_LISTINGS_PER_LEAGUE`, `ai-listings.ts:25`), `incident_id`, úroveň a stav z `loss`. Id inzerátu je neprůhledné: `bazar-` a prvních 32 hex znaků SHA-256 z `{incidentId}|{kategorie}` (`idInzeratu`, `bazar-db.ts`), ne prosté `bazar-{incidentId}-{kategorie}` (id v URL by prozradilo okradený klub). Pořád deterministické, `INSERT OR IGNORE` tak nevystaví dvakrát.
- **Prodejce:** `seller_name` v 1. pádě jako „Láďa, Volary", generátor jméno obce spolehlivě neskloňuje. Obec náhodně z `villages` okresu ligy (u rezerv U21 se z názvu okresu odstraní přípona „U21"); když okres žádnou obec nemá, záložní seznam (Lhota, Újezd, Dvory, Zálesí). Na kartě štítek „soukromý inzerát" místo „z okolí", proto to samo jméno prodejce neopakuje.
...
- **Poznání:** `jePoznatelne(kategorie, uroven)` — dresy ≥ 2 (čísla, logo), dodávka 3 (logo), vitrína ≥ 1 (poháry s nápisy), bubny a vlajky ≥ 2 (šály s logem). Ostatní nepozná nikdo.
  - poznatelné → okradený klub dostane u inzerátu `vypadaJakoVase: true`, SMS od Kustoda (text nezmiňuje policii, chodí i u incidentů, kde se nahlásit nedá) a (jen u neuzavřeného incidentu) stopu `bazar` s bonusem pro policii 0,3 (7c). Stopa platí dál i po stažení nebo prodeji inzerátu.
  - nepoznatelné → nic; nápovědou je jen cena a načasování.
- **Nahlásit policii** (jen poznatelné, jen okradený klub, jen když má nahlášení smysl podle `lzeNahlasit`, `bazar.ts`): tlačítko i endpoint se řídí stejným pravidlem jako `dostupneAkce.policie` s výjimkou probíhajícího šetření. Jde to, když policie na incidentu ještě nešetřila (pachatel neodhalen, žádný dřívější výsledek), nebo právě šetří a pachatel není odhalený. Odhalený pachatel nahlášení odmítne i během šetření (udání vlastního hráče, 7d, nechá incident ve stavu `policie` s `culprit_revealed = 1`). Pořadí: nejdřív se zavolá policie (7c), a teprve po jejím úspěchu se inzerát stáhne (`withdrawn`). Je-li ho mezitím pryč, protože ho koupil někdo jiný, zaloguje se to a nahlášení skončí úspěchem, policie případ má. Když policie právě šetří, inzerát se jen zajistí (`withdrawn`, SMS od policie), samotné šetření běží dál beze změny. Po skončeném šetření, u odhaleného pachatele nebo u uzavřeného incidentu nahlášení vrátí 409 a inzerát v bazaru zůstane. Policie řeší jednu krádež jen jednou (7c).
- **Koupit zpět:** běžný nákup. Hook v `equipment-market.ts` buy: `listing.incident_id` a kupec = okradený klub → `recovered = 1` vždy. SMS od Kustoda jen u poznatelného zboží, protože u nepoznatelného by prozradila vlastnictví, které klub jinak neměl jak poznat. Incident zůstává ve svém stavu, pachatel pořád není známý a vyšetřování běží dál.
...
GET `/equipment-market` vrací navíc `isPrivateListing`, `vypadaJakoVase`, `incidentId` a `lzeNahlasit`. Poslední dva jen okradenému klubu a jen u poznatelného zboží, jinak by samotné id u nepoznatelného zboží prozradilo, že jde o jeho věci. Vlastnictví pro tenhle výpočet se neurčuje podle `:teamId` z URL (to si podvrhne kdokoli, `requireTeamOwnership` GET pouští bez kontroly), ale ze session (`tymyDivaka`, stejně jako `routes/incidents.ts`).
